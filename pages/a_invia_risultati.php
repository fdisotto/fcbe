	...
	<table width='100%' cellpadding='0' cellspacing='0' bgcolor='$sfondo_tab'><caption>$soprannome_squadra di $outente</caption><tr><td><font size='-2'><u>Calciatore</u></font></td><td align='center'><font size='-2'>Fanta<br/>voto</font></td><td align='center'><font size='-2'>Voto<br/>giornale</font></td></tr>";
        ...
        if ( $voti_esistenti == "SI" ) {
            ...
            if ( $voti_bonus_in_casa )
                $mexemail .= "<br/><table align=\"center\" bgcolor=\"$sfondo_tab\" class=\"border\" cellpadding=\"10\"><tr><td align=\"left\" valign=\"top\"><b><u>Voti della giornata $ggult</u></b>:<br/><font color=\"red\">(Aggiungere +$voti_bonus_in_casa per le squadre in casa)</font><br/><br/>"; else
    ...
    TD.testa {
        ...
        if ( $gestione_email == "mail_OK" ) {
            ...
            if ( @mail( "Utenti $titolo_sito <$email_mittente>", $oggetto, "$mexemail\r\n", $intestazioni ) )
                $azione = "La mail &egrave; stata inoltrata con successo."; else
                $azione = "Si sono verificati dei problemi nell'invio della mail.";
        } elseif ( $gestione_email == "mail_anteprima" )
            $azione = "<center><h3>Invio resoconto ultima giornata</h3><form method=\"post\" action=\"a_invia_risultati.php\">
...
if ( $_SESSION[ 'permessi' ] == 5 ) {
    ...
    if ( isset( $gestione_email ) && ( $gestione_email == "mail_anteprima" or $gestione_email == "mail_OK" ) ) {
        for ( $num1 = 1; $num1 < 40; $num1++ ) {
            ...
            if ( ! @is_file( "$percorso_cartella_dati/$giornata_controlla" ) ) {
                break;
            } else {
                $giornata_ultima = $num1 . $pp;
            }
        } # fine for $num1

        if ( ! isset( $giornata ) or $giornata > $giornata_ultima ) {
            $giornata = $giornata_ultima ?? "00";
        }
        ...
        if ( isset( $giornata_ultima ) ) {
            $file_giornata = file( "$percorso_cartella_dati/giornata$giornata" );
        }
    ...
    ###############################
        ...
        $azione = "<h3>Invio resoconto ultima giornata</h3>
         $frase<br>
        <form method=\"post\" action=\"a_invia_risultati.php\">
        <input type=\"hidden\" name=\"gestione_email\" value=\"mail_anteprima\">
