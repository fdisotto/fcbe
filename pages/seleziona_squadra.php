...
<caption>Modifica squadra</caption><tr><td align='center'>";
        ...
        if ( $proprietario == $_SESSION[ 'utente' ] ) {
            ...
            if ( $sec_restanti < 0 ) {
                ...
                if ( isset( $$nome_schierato ) && $$nome_schierato == "panchinaro" ) {
                    ...
                    if ( isset( $num_in_panchina_usati[ $num_in_panchina ] ) ) {
                        $inserire = "NO";
    ...
    if ( $num_titolari_F != 0 )
        $schema = $num_titolari_P . $num_titolari_D . $num_titolari_C . $num_titolari_F . $num_titolari_A;
    $schema_trovato = "NO";
    if ( $num_titolari > 11 ) {
        $inserire = "NO";
    ...
    if ( $num_titolari < 11 ) {
        ...
        for ( $num1 = 0; $num1 < $num_schemi; $num1++ ) {
            ...
            if ( $num_cons_P > $max_num_cons_P ) {
                $max_num_cons_P = $num_cons_P;
            }
            if ( $num_cons_D > $max_num_cons_D ) {
                $max_num_cons_D = $num_cons_D;
            }
            if ( $num_cons_C > $max_num_cons_C ) {
                $max_num_cons_C = $num_cons_C;
            }
            if ( $num_cons_F > $max_num_cons_F ) {
                $max_num_cons_F = $num_cons_F;
            }
            if ( $num_cons_A > $max_num_cons_A ) {
                $max_num_cons_A = $num_cons_A;
            }
        } # fine for $num1

        if ( $num_titolari_P > $max_num_cons_P ) {
            $inserire = "NO";
            $frase .= "Troppi portieri!<br />";
        }
        if ( $num_titolari_D > $max_num_cons_D ) {
            $inserire = "NO";
            $frase .= "Troppi difensori!<br />";
        }
        if ( $num_titolari_C > $max_num_cons_C ) {
            $inserire = "NO";
            $frase .= "Troppi centrocampisti!<br />";
        }
        if ( $num_titolari_F > $max_num_cons_F ) {
            $inserire = "NO";
            $frase .= "Troppi fantasisti!<br />";
        }
        if ( $num_titolari_A > $max_num_cons_A ) {
            $inserire = "NO";
            $frase .= "Troppi attaccanti!<br />";
        }
    } # fine if ($num_titolari < 11)
