...
if ( $iscrizione_online == "SI" ) {
    if ( ! isset( $_POST[ 'inserimento' ] ) || $_POST[ 'inserimento' ] != "ok" ) {
        ...
        for ( $num1 = 1; $num1 < $num_tornei; $num1++ ) {
            ...
            $fileo = @file( DATI_DIR . "/utenti_" . $otid . ".php" );
            $linee = @count( $fileo );
            $num_giocatori = 0;
        ...
        if ( $num1 <= 1 )
            $erro = "<b>Nessun Torneo attivo</b>"; else $erro = "<input type = 'image' src = 'immagini/registrati.png' name = 'submit' alt = 'Prosegui e inserisci' />"
        ...
        <form method="post" action="iscrizione.php">
            ...
            <table summary="Modulo iscrizione">
                ...
                <tr>
                    ...
                    <td width="75%">
                        <input type="text" name="isquadra"/>&nbsp;&nbsp; obbligatorio minimo 4 e massimo 18 caratteri
                    </td>
                </tr>
                <tr>
        ...
        <?php
    } elseif ( isset( $_POST[ 'inserimento' ] ) && $_POST[ 'inserimento' ] == "ok" ) {
        ####################################################
        ...
        //-----------------------------------------
        if ( ! @is_file( DATI_DIR . "/utenti_" . $_POST[ 'itorneo' ] . ".php" ) ) {
            $ini_file = "<?php die('ACCESSO VIETATO');?> // password = 5f4dcc3b5aa765d61d8327deb882cf99  --> md5(password)\n";
            $fp = fopen( DATI_DIR . "/utenti_" . $_POST[ 'itorneo' ] . ".php", "wb" ) or die ( "errore fileopen" );
        ...
        for ( $num1 = 1; $num1 < $num_tornei; $num1++ ) {
            ...
            if ( @is_file( DATI_DIR . "/utenti_" . $itorneo . ".php" ) ) {
                $filep = @file( DATI_DIR . "/utenti_" . $itorneo . ".php" ) or die( "Ci sono problemi di permessi sul file utenti" );
                $linee = count( $filep );
                $trovato = 0;
        ...
        if ( ! empty( $err ) ) {
            ...
            if ( $attivazione_email ) {
                ...
                if ( ! file_exists( DATI_DIR . "/hash.php" ) ) {
                    $filehash = fopen( DATI_DIR . "/hash.php", "w+" );
                ...
                $filehash = fopen( DATI_DIR . "/hash.php", "ab" );
                flock( $filehash, LOCK_EX );
...
<hr />";
            ...
            $fp = fopen( DATI_DIR . "/utenti_" . $_POST[ 'itorneo' ] . ".php", "ab" );
            flock( $fp, LOCK_EX );
            fwrite( $fp, $stringa );
            flock( $fp, LOCK_UN );
            fclose( $fp );
...
<caption>Iscrizione utente al campionato</caption>
<tr><td align='center'><h1>Si è verificato un problema.</h1> <br /><br /><br /><br /><br /><br /></td></tr></table>
