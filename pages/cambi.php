...
if ( $_SESSION[ 'valido' ] == "SI" ) {
    ...
    for ( $num1 = 1; $num1 < 40; $num1++ ) {
        ...
        if ( @is_file( DATI_DIR . "/giornata" . $num1 . "_" . $_SESSION[ 'torneo' ] . "_" . $_SESSION[ 'serie' ] ) )
            $ultima_giornata = $num1; else break;
    } # fine for $num1

    if ( $diff_num_giornata_file > 0 )
        $voti_ultima_giornata = $ultima_giornata + $diff_num_giornata_file; else $voti_ultima_giornata = $ultima_giornata;
                ...
                if ( count( $dati_calciatore ) >= 6 ) {
                    ...
                    if ( $proprietario == $nome_squadra ) {
                        ...
                        if ( $ultima_giornata > 0 )
                            $percorso = "$percorso_cartella_voti/voti$voti_ultima_giornata.txt"; else $percorso = "dati/calciatori.txt";
					...
					<input type='hidden' name='val_cal' value='$valutazione' />"; #$offri = "<a href='valuta.php?ins_val_ces=SI&amp;num_cal=$numero&amp;nome=$nome&amp;ruolo=$ruolo&amp;costo=$costo&amp;val_cal=$valutazione' class='user'>seleziona</a>";
                        ...
                        if ( $ruolo == "D" or $ruolo == "A" )
                            $bgcolore = "$colore_riga_alt"; else $bgcolore = "#FFFFFF";
	...
	$tab_comprati</table></td><td width='50%' valign='top' align='center'>";
            ...
            $filep = file( DATI_DIR . "/utenti_" . $_SESSION[ 'torneo' ] . ".php" );
            ...
            if ( $rip_cambi_numero != 0 ) {
                ...
                for ( $num1 = 0; $num1 < $numgiorip; $num1++ ) {
                    if ( $ultima_giornata == $rip_cambi_giornate[ $num1 ] or $ultima_giornata == ( $rip_cambi_giornate[ $num1 ] + $rip_cambi_durata ) ) {
                        ...
                        if ( @is_file( DATI_DIR . "/cce_" . $_SESSION[ 'torneo' ] . "_" . $_SESSION[ 'serie' ] . ".txt" ) ) {
                            $controllo_cambi_extra = @file( DATI_DIR . "/cce_" . $_SESSION[ 'torneo' ] . "_" . $_SESSION[ 'serie' ] . ".txt" );
		...
		<input type='text' name='num_ricerca' size='3' /></form>";
            ...
            if ( $num_ricerca ) {
                if ( $ultima_giornata > 0 )
                    $percorso = $percorso_cartella_voti . "/voti" . $voti_ultima_giornata . ".txt"; else $percorso = DATI_DIR . "/calciatori.txt";
            ...
            if ( @is_file( DATI_DIR . "/cambi_$nome_squadra" ) )
                $vedi_valuta = @file( DATI_DIR . "/cambi_$nome_squadra" );
		...
		<td class='testa'>Saldo</td></tr>";
            ...
            if ( @is_file( DATI_DIR . "/cambi_$nome_squadra" ) ) {
                $controllo = (int)@file( DATI_DIR . "/cambi_$nome_squadra" );
                if ( $controllo != 0 )
                    echo "<br/><br/><a href='valuta.php?eli_tutti=SI' class='user'><b>X</b></a> - Elimina tutti<br/><br/><font class='evidenziato'>Attenzione che la conferma dei cambi &egrave; definitiva ed irreversibile.<br/>La valutazione resta comunque memorizzata fino alla giornata successiva.</font>
		<center>
