...
if ( $_SESSION[ 'valido' ] == "SI" and $_SESSION[ 'permessi' ] >= 0 ) {
    ...
    if ( isset( $_GET[ 'cambia' ] ) or $_SESSION[ 'permessi' ] >= 4 )
        $id = $_GET[ 'cambia' ]; else $id = $_SESSION[ 'uid' ];

    if ( $id ) {
        if ( isset( $_GET[ 'go' ] ) ) {
            if ( $_SESSION[ 'permessi' ] == 5 )
                $id_torneo = $_GET[ 'id_torneo' ]; else $id_torneo = $_SESSION[ 'torneo' ];
            ...
            if ( $id == 0 ) {
                ...
                $percorso_file = DATI_DIR . "/utenti_$id_torneo.php";
                ...
                if ( isset( $_POST[ "ipermessi" ] ) && ( $_POST[ "ipermessi" ] or $_POST[ "ipermessi" ] == "0" ) )
                    $Npermessi = $_POST[ "ipermessi" ]; else $Npermessi = $opermessi;
                ...
                if ( isset( $_POST[ "icrediti" ] ) ) {
                    $Ncrediti = $_POST[ "icrediti" ];
                } else {
                    $Ncrediti = "$ocrediti";
                }

                if ( isset( $_POST[ "ivariazioni" ] ) ) {
                    $Nvariazioni = $_POST[ "ivariazioni" ];
                } else {
                    $Nvariazioni = "$ovariazioni";
                }

                if ( isset( $_POST[ "icambi" ] ) ) {
                    $Ncambi = $_POST[ "icambi" ];
                } else {
                    $Ncambi = "$ocambi";
                }

                if ( isset( $_POST[ "ilogo" ] ) ) {
            ...
            if ( $_SESSION[ 'permessi' ] == 5 )
                $id_torneo = $_GET[ 'itorneo' ]; else $id_torneo = $_SESSION[ 'torneo' ];
            ...
            for ( $num1 = 1; $num1 < $num_tornei; $num1++ ) {
                ...
                if ( $itorneo == $tid )
                    $vedi_tornei_attivi .= "<option value='$tid' selected>$tdenom</option>"; else $vedi_tornei_attivi .= "<option value='$tid'>$tdenom</option>";
            } # fine for $num1
                ...
                <table summary="Modifica utente" width="100%" bgcolor="<?php echo "$sfondo_tab"; ?>" cellpadding="5" align="center">
                    ...
                    <tr align="left">
                        ...
                        <td width="70%" valign="bottom">
                            ...
                            if ( $_SESSION[ 'permessi' ] >= 4 )
                                echo "<select name='ipermessi'>"; else echo "<select name='ipermessi' DISABLED>";

                            if ( $ipermessi == "-1" )
                                echo "<option value='-1' SELECTED> -1 </option>"; else echo "<option value='-1'> -1 </option>";

                            if ( $ipermessi == "0" )
                                echo "<option value='0' SELECTED> 0 </option>"; else echo "<option value='0'> 0 </option>";

                            if ( $ipermessi == "1" )
                                echo "<option value='1' SELECTED> 1 </option>"; else echo "<option value='1'> 1 </option>";

                            if ( $ipermessi == "2" )
                                echo "<option value='2' SELECTED> 2 </option>"; else echo "<option value='2'> 2 </option>";

                            if ( $ipermessi == "3" )
                                echo "<option value='3' SELECTED> 3 </option>"; else echo "<option value='3'> 3 </option>";

                            if ( $ipermessi == "4" )
                                echo "<option value='4' SELECTED> 4 </option>"; else echo "<option value='4'> 4 </option>";

                            if ( $_SESSION[ 'permessi' ] == 5 ) {
                                if ( $ipermessi == "5" )
                                    echo "<option value='5' SELECTED> 5 </option>"; else echo "<option value='5'> 5 </option>";
                            }
                            ?>
                            </select>
                        </td>
                    ...
                    <tr align="left">
                        ...
                        <td width="70%" valign="bottom">
                            <input type="text" name="iurl" value="<?php if ( $iurl )
                                echo "$iurl"; ?>"/></td>
                    </tr>
                    <?php
                    ...
                    if ( $consenti_logo == "SI" and $_SESSION[ 'permessi' ] <= 4 ) {
                        ...
                        $nomeimg = '';
                        if ( file_exists( PUBLIC_DIR . "/immagini/loghi/" . $_SESSION[ 'utente' ] . ".jpg" ) )
                            $nomeimg = "/immagini/loghi/" . $_SESSION[ 'utente' ] . ".jpg";
                        if ( file_exists( PUBLIC_DIR . "/immagini/loghi/" . $_SESSION[ 'utente' ] . ".gif" ) )
                            $nomeimg = "/immagini/loghi/" . $_SESSION[ 'utente' ] . ".gif";

                        if ( file_exists( PUBLIC_DIR . $nomeimg ) ) {
                        ...
                        $nomeimg = "";
                        if ( file_exists( PUBLIC_DIR . "/immagini/loghi/" . $iutente . ".jpg" ) ) {
                            $nomeimg = "/immagini/loghi/" . $iutente . ".jpg";
                        }
                        if ( file_exists( PUBLIC_DIR . "/immagini/loghi/" . $iutente . ".gif" ) ) {
                    ...
                    if ( $_SESSION[ 'permessi' ] >= 3 )
						...
						</tr>"; else echo "<input type = 'hidden' name = 'isquadra' value = '$isquadra'>";
                    ?>
                    <tr align="left">
        ...
        <table summary='Modifica utente' bgcolor="<?php echo "$sfondo_tab"; ?>" cellpadding="10" width="100%" align="center">
            ...
            <tr>
                <td valign="top" style="border: 3px double #888888;">
                    <?php
                    if ( $_SESSION[ 'permessi' ] == 5 )
                        $percorso_file = DATI_DIR . "/utenti_$itorneo.php"; else $percorso_file = DATI_DIR . "/utenti_" . $_SESSION[ 'torneo' ] . ".php";
