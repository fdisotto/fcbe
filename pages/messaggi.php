...
if ( $_SESSION[ 'valido' ] == "SI" ) {
    ...
    if ( $_SESSION[ 'permessi' ] >= 4 and $_SESSION[ 'valido' ] == "SI" ) {
        if ( isset( $b ) && $b != 0 ) {
            unlink( DATI_DIR . "/messaggi/$b.php" );
        }
    }
    $b = 0;
    #####fine cancella mex####
    ...
    if ( $_SESSION[ 'permessi' ] >= 4 and $_SESSION[ 'valido' ] == "SI" ) {
        if ( isset( $c ) && $c != 0 ) {
            unlink( DATI_DIR . "/messaggi/$c.php" );
        }
    }
    $c = 0;
    #####fine cancella mex####

    if ( isset( $usa_tinyMCE ) && $usa_tinyMCE == "SI" ) {
    ...
    function main()
    {
        ...
        echo "<table summary='' width='100%' bgcolor='$sfondo_tab'>
		<tr>
		<th class='testa'>Oggetto</th>
		<th class='testa' width='70'>Risposte</th>
		<th class='testa' width='150'>Messaggio iniziale</th>
		<th class='testa' width='150'>Ultimo messaggio</th>";
        ...
        for ( $x = $page * $messxpagina; $x < $messxpagina * ( $page + 1 ); $x = $x + 1 ) {
            ...
            if ( $file != "" ) {
                ...
                if ( $otmessaggi == 1 and $otdenom == trim( $topicinfo[ 5 ] ) ) {
                    ...
                    <td align='center' width='70'>&nbsp;<?php echo count( $topic ); ?>&nbsp;</td>
                    ...
                    <tr bgcolor='$colore'>
                        ...
                        if ( $_SESSION[ 'permessi' ] >= 4 and $_SESSION[ 'valido' ] == "SI" ) {
                            ...
                            <td align='center' width='150'><a href="messaggi.php?b=<?php echo substr( $file, 0, strlen( $file ) - 4 ); ?>" class='user'><?php echo "Cancella"; ?></a><br/>
                                <a href="a_edita_file.php?mod_file=./dati/messaggi/<?php echo substr( $file, 0, strlen( $file ) - 4 ); ?>.php" class='user'><?php echo "Modifica"; ?></a><br/></td> <?php
                        } ?>
                    </tr>
        ...
        for ( $thispagenum = 1; $thispagenum <= ceil( count( $files ) / $messxpagina ); $thispagenum = $thispagenum + 1 ) {
            $pages .= "[<a href='messaggi.php?page=" . $thispagenum . "' class='user1'>" . $thispagenum . "</a>]";
        }
    ...
    if ( isset( $a ) && $a == "newtopic" ) {
        newtopic();
    }
    if ( isset( $a ) && $a == "newtopic2" ) {
        newtopic2();
    }
    echo "</td></tr></table>";
} else {
