...
if ( $_SESSION[ 'valido' ] == "SI" and $_SESSION[ 'permessi' ] == 5 ) {
    ...
    if ( isset( $inserimento ) && $inserimento == "scrivi" ) {
        if ( $azione == "nuovo" ) {
            ...
            $fp = fopen( DATI_DIR . "/tornei.php", "a+" );
            flock( $fp, LOCK_SH );
            fwrite( $fp, $stringa );
            flock( $fp, LOCK_UN );
            fclose( $fp );
            ...
            $fo = fopen( DATI_DIR . "/tornei.php", 'wb+' );
            flock( $fo, LOCK_EX );
        ...
        if ( ! isset( $itorneo ) ) {
            if ( ! @is_file( DATI_DIR . "/tornei.php" ) ) {
                ...
                $fp = fopen( DATI_DIR . "/tornei.php", "wb+" ) or die ( "errore fileopen" );
                ...
                if ( @chmod( DATI_DIR . "/tornei.php", 0664 ) )
                    echo "CHMOD 664 impostato!<br />";
                unset ( $ini_file, $fp );
            }
            $dati_tornei = @file( DATI_DIR . "/tornei.php" );
