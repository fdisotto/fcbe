...
##################################################################################
require_once PAGES_DIR . "/controlla_pass.php";
...
if ( $_SESSION[ 'valido' ] == "SI" and $_SESSION[ 'permessi' ] == 5 ) {
    require_once PAGES_DIR . "/a_menu.php";

    unset ( $tabella_msg );

    if ( isset( $messgestutente ) ) {
        require_once INC_DIR . "/avvisi.php";
    ...
    if ( $otmercato_libero == "SI" )
        $mess1 = "Il torneo si volge a mercato libero, che significa che tutti i giocatori possono acquistare qualsiasi calciatore, indipendentemente dal fatto che possa essere stato acquistato da altri giocatori; si dispongono di <b>$otnumero_cambi_max</b> cambi di calciatore per tutte la stagione, oltre alle eventuali giornate di riparazione."; elseif ( $otmercato_libero == "NO" )
        $mess1 = "Il torneo si volge con una asta iniziale, durante la quale vendono assegnati i calciatori al maggior offerente.";
    else $mess1 = "<div class='evidenziato'>ERRORE: il tipo di mercato non &egrave; valido.</div>";

    $mess2 = "Tipo calcolo: $ottipo_calcolo";
    $mess3 = "Stato: $otstato";

    if ( file_exists( DATI_DIR . "/mercato_" . $otid . "_" . $otserie . ".txt" ) )
        $dati_mercato = @file( DATI_DIR . "/mercato_" . $otid . "_" . $otserie . ".txt" ); else unset( $dati_mercato );
    ...
    $dati_utenti = @file( DATI_DIR . "/utenti_" . $itorneo . ".php" );
	...
	<td><img src='immagini/828.gif' alt='Voti' width='32' height='32' /></td>";
    if ( $otstato == "B" )
        echo "<td><img src='immagini/email.gif' alt='Apri buste' width='32' height='32' /></td>";
    echo "</tr>
	<tr align='center' valign='middle'>
	...
	<td>Edita<br/>
	<a href='./a_edita_file.php?mod_file=" . DATI_DIR . "/utenti_" . $itorneo . ".php'><b>U</b></a> -
	<a href='./a_edita_file.php?mod_file=" . DATI_DIR . "/mercato_" . $itorneo . "_0.txt'><b>M</b></a> -
	<a href='./a_edita_file.php?mod_file=" . DATI_DIR . "/registro_mercato_" . $itorneo . "_0.txt'><b>R</b></a> -
	<a href='./a_edita_file.php?mod_file=" . DATI_DIR . "/log" . $itorneo . ".txt'><b>L</b></a>
	</td>";
    if ( $otstato == "B" )
        echo "<td><a href='a_gestione_tornei.php?itorneo=$itorneo&amp;opzione=6'>Buste</a></td>";

    if ( $otstato == "B" )
        echo "</tr><tr><td colspan='6' align='left'>"; else echo "</tr><tr><td colspan='5' align='left'>";
    echo "<br/><br/><br/><b><u>Riepilogo informazioni:</u/></b><br/>
	Ci sono <b>" . count( $a_utenti ) . "</b> giocatori registrati.<br/>
	$mess1<br/>
	$mess2<br/>
	$mess3<br/>
	<div style='color: #DDDDDD; text-align: right'>$itorneo " . ( isset( $opzione ) ? $opzione : '' ) . "</div>
	</td></tr>
	</table>";

    unset ( $d, $dati );

    if ( isset( $opzione ) && $opzione == 1 ) {
        $file = @file( DATI_DIR . "/utenti_" . $itorneo . ".php" );
        $linee = count( $file );
        echo "<table bgcolor='$sfondo_tab' width='100%' cellpadding='1' cellspacing='0'>
		<caption>Utenti iscritti al torneo <b>$otdenom</b></caption>";
        ...
        for ( $num1 = 1; $num1 < $linee; $num1++ ) {
            ...
            if ( $ourl and $ourl != "http://" )
                $link_sito = "&nbsp;&nbsp;&nbsp;(<a href='$ourl' target='_blank'>Sito web</a>)"; else unset( $link_sito );
            if ( $num1 % 2 )
                $colore = "white"; else $colore = $colore_riga_alt;

            if ( isset( $a_mercato ) ) {
                foreach ( $a_mercato as $dati ) {
                    if ( isset( $dati[ $outente ] ) ) {
            ...
            $ums = "";
            if ( file_exists( DATI_DIR . "/squadra_$outente" ) )
                $ums = "<br />- modifica formazione: " . date( "d-m-Y H:i:s.", filemtime( DATI_DIR . "/squadra_" . $outente ) ); else $ums = "&nbsp;";
			...
			<td align='left'>$osquadra (" . $a_utenti[ $outente ][ 10 ] . ")</td>
			...
			<td align='center'><img src='./immagini/no.gif' style='vertical-align: middle' alt='Cancella utente' />&nbsp;&nbsp;<a href='a_eliUtente.php?del=$num1&amp;itorneo=$itorneo&amp;gt=1&amp;oemail=$oemail'> Cancella</a></td>
    ...
    elseif ( isset( $opzione ) && $opzione == 2 ) {
        $filei = @file( DATI_DIR . "/utenti_" . $itorneo . ".php" );
        $linee = count( $filei );
        ...
        $messaggi = @file( DATI_DIR . "/registro_mercato_" . $itorneo . "_0.txt" );
        ...
        for ( $num1 = $num_iniziale; $num1 < $num_messaggi; $num1++ ) {
            ...
            if ( substr( "$messaggi[$num1]", 0, 13 ) == "Radio mercato" and ( ! isset( $stato_mercato ) || $stato_mercato != "I" ) )
                $messmerc .= "$nome<br/>";
        } # fine for $num1
        echo "<table width='100%' align='center' class='border' cellpadding='10' bgcolor='$sfondo_tab'><caption>Registro Mercato</caption><tr valign ='top'><td align='left'>
		$messmerc
		</td></tr></table>";
    } #######################
    elseif ( isset( $opzione ) && $opzione == 3 ) {
        $nome_squadra = "tutti";
        $file = @file( DATI_DIR . "/utenti_" . $itorneo . ".php" );
        $linee = count( $file );
        ...
        for ( $num1 = 1; $num1 < $linee; $num1++ ) {
            ...
            $linea_comprato_F = "";
            $linea_comprato_A = "";
            $calciatori = @file( DATI_DIR . "/mercato_" . $itorneo . "_0.txt" );
            $np = 0;
            $nd = 0;
            $nc = 0;
            $nf = 0;
            ...
            $dati_squadra = @file( DATI_DIR . "/squadra_" . $outente );
            ...
            for ( $num2 = 0; $num2 < $num_panchinari; $num2++ ) {
                ...
                if ( isset( $linea_comprato_P[ $numero_panchinaro ] ) ) {
                    ...
                    $linea_comprato_P[ $numero_panchinaro ] = str_replace( "<tr bgcolor='$colore'><td>&nbsp;</td>", "<tr bgcolor='$colore'><td align='center'>$num_pos</td>", $linea_comprato_P[ $numero_panchinaro ] );
                    $num_pos++;
                ...
                if ( isset( $linea_comprato_D[ $numero_panchinaro ] ) ) {
                    ...
                    $linea_comprato_D[ $numero_panchinaro ] = str_replace( "<tr bgcolor='$colore'><td>&nbsp;</td>", "<tr bgcolor='$colore'><td align='center'>$num_pos</td>", $linea_comprato_D[ $numero_panchinaro ] );
                    $num_pos++;
                ...
                if ( isset( $linea_comprato_C[ $numero_panchinaro ] ) ) {
                    ...
                    $linea_comprato_C[ $numero_panchinaro ] = str_replace( "<tr bgcolor='$colore'><td>&nbsp;</td>", "<tr bgcolor='$colore'><td align='center'>$num_pos</td>", $linea_comprato_C[ $numero_panchinaro ] );
                    $num_pos++;
                ...
                if ( isset( $linea_comprato_F[ $numero_panchinaro ] ) ) {
                    ...
                    $linea_comprato_F[ $numero_panchinaro ] = str_replace( "<tr bgcolor='$colore'><td>&nbsp;</td>", "<tr bgcolor='$colore'><td align='center'>$num_pos</td>", $linea_comprato_F[ $numero_panchinaro ] );
                    $num_pos++;
                ...
                if ( isset( $linea_comprato_A[ $numero_panchinaro ] ) ) {
                    ...
                    $linea_comprato_A[ $numero_panchinaro ] = str_replace( "<tr bgcolor='$colore'><td>&nbsp;</td>", "<tr bgcolor='$colore'><td align='center'>$num_pos</td>", $linea_comprato_A[ $numero_panchinaro ] );
                    $num_pos++;
    ...
    elseif ( isset( $opzione ) && $opzione == 4 ) {
        ...
        # Numero minimo di calciatori che devono essere titolari in formazione per ottenere punti (sono compresi anche quelli che non giocano). Non impostare a pi� di 11.
        $min_num_titolari_in_formazione = $otmin_num_titolari_in_formazione;

        $punti_pareggio = $otpunti_pareggio;            # impostare a "M" per la media, "A" per i punti della posizione pi� alta o "B" per quelli della pi� bassa
        $punti_posizione = array();
        $punti_posizione = explode( "-", $otpunti_pos );    # punti assegnati al primo di giornata
        #################################################################################################

        echo "<div style='background-color: $sfondo_tab; margin-top:5px; margin-bottom:5px; padding: 5px; border: 1px solid $sfondo_tab2'>Giornate giocate:&nbsp;";
        ...
        for ( $num1 = 1; $num1 < 40; $num1++ ) {
            ...
            if ( ! @is_file( DATI_DIR . "/" . $giornata_controlla . "_" . $itorneo . "_0" ) )
                break; else {
        ...
        $file_giornata = [];
        if ( $giornata_ultima ) {
            $file_giornata = @file( DATI_DIR . "/giornata" . $giornata . "_" . $itorneo . "_0" );
        }
        ...
        for ( $num1 = 1; $num1 < $linee; $num1++ ) {
            ...
            if ( $opermessi != -1 ) {
				...
				<table width='100%' cellpadding='3' cellspacing='1' bgcolor='$sfondo_tab' align='center'>
        ...
        if ( $voti_esistenti == "SI" ) {
            ...
            if ( $tipo_campionato != "N" ) {
                ...
                for ( $num1 = $g_inizio_campionato; $num1 <= $num_giornata; $num1++ ) {
                    ...
                    if ( @is_file( DATI_DIR . "/" . $giornata_punti . "_" . $itorneo . "_0" ) ) {
                        $file_giornata_p = @file( DATI_DIR . "/" . $giornata_punti . "_" . $itorneo . "_0" );
                    ...
                    } # fine if (@is_file(DATI_DIR . "/$giornata_punti"))
                } # fine for $num1
    ...
    } elseif ( isset( $opzione ) && $opzione == 5 ) {
        $file = @file( DATI_DIR . "/utenti_" . $itorneo . ".php" );
        $linee = count( $file );
        echo "<table bgcolor='$sfondo_tab' width='100%' cellpadding='1' cellspacing='0'>
        ...
        echo "</table>";
    } elseif ( isset( $opzione ) && $opzione == 6 ) {
    ...
    } elseif ( isset( $buste ) && $buste == 1 ) {
        if ( $_SESSION[ 'valido' ] == "SI" and $_SESSION[ 'permessi' ] == 5 ) {
            // Variabili da impostare
            $filename = DATI_DIR . "/mercato_" . $itorneo . "_0.txt"; // Path completo del file
            ...
            $data_buste_chiuse = @join( '', file( DATI_DIR . "/data_buste_" . $itorneo . "_0.txt" ) );
            $data_busta_precedente = @join( '', @file( "./dati/data_buste_precedente_" . $itorneo . "_0.txt" ) );
            //
            $lista_offerte_identiche = [];
            $lista_calciatori = [];
            $box_data->data = "";
            $box_data->valore = 0;
            ...
            $file_messaggi = fopen( DATI_DIR . "/registro_mercato_" . $itorneo . "_0.txt", "ab+" );
            ...
            while ( ! feof( $fd ) ) {
                ...
                $busta_vecchia = 0;
                if ( $cur_date == $data_busta_precedente )
                    $busta_vecchia = 1;
                if ( isset ( $lista_calciatori[ $cur_id ] ) ) {
                    if ( $cur_value > $lista_calciatori[ $cur_id ][ $index_value ] ) {
                        ...
                        if ( $busta_vecchia == 0 ) {
                            flock( $file_messaggi, LOCK_EX );
                            fwrite( $file_messaggi, $messaggio . " offerto " . $lista_calciatori[ $cur_id ][ $index_value ] . " FantaMilioni per " . $lista_calciatori[ $cur_id ][ $index_name ] . " parit� con " . $lista_calciatori[ $cur_id ][ $index_proprietario ] . " [Giocatore reinserito nel mercato]\n" );
                            flock( $file_messaggi, LOCK_UN );
                        }
            ...
            $fd = fopen( DATI_DIR . "/buste_aperte_" . $itorneo . "_0.txt", "w" );
            ...
            $newfile = DATI_DIR . "/buste_chiuse_" . $itorneo . "_0.txt";
            $aperte = DATI_DIR . "/buste_aperte_" . $itorneo . "_0.txt";
            copy( $filename, $newfile );
            unlink( $filename );
            copy( $aperte, $filename );
            $file_data_buste = fopen( DATI_DIR . "/data_buste_precedente_" . $itorneo . "_0.txt", "w" );
