...
if ( $_SESSION[ 'valido' ] == "SI" and $_SESSION[ 'utente' ] != $admin_user ) {
    require_once PAGES_DIR . "/menu.php";
    ...
    $nf = 0;
    $na = 0;
    $calciatori = @file( DATI_DIR . "/mercato_" . $_SESSION[ 'torneo' ] . "_" . $_SESSION[ 'serie' ] . ".txt" ) ?: [];
    ...
    for ( $num1 = 0; $num1 < $num_calciatori; $num1++ ) {
        ...
        if ( count( $dati_calciatore ) >= 6 ) {
            ...
            if ( $sec_restanti < 1 or ( $stato_mercato != "I" and $stato_mercato != "P" ) ) {
                ...
                if ( $_SESSION[ 'utente' ] == $proprietario and $mercato_libero != "SI" and $stato_mercato == "B" )
                    $offri = "<a href='busta_vendi_subito.php?num_calciatore=$numero' class='user'>svincola</a>";
                if ( $_SESSION[ 'utente' ] != $proprietario and $mercato_libero != "SI" and $stato_mercato == "B" )
                    $offri = "<a href='busta_offerta.php?num_calciatore=$numero&amp;altro_utente=$proprietario' class='user'>offerta</a>";
                if ( $_SESSION[ 'utente' ] == $proprietario and $mercato_libero != "SI" and $stato_mercato != "I" and $stato_mercato != "C" )
                    $offri = "<a href='vendi.php?num_calciatore=$numero' class='user'>vendi</a>";
                if ( $_SESSION[ 'utente' ] != $proprietario and $mercato_libero != "SI" and $stato_mercato != "I" and $stato_mercato != "C" )
                    $offri = "<a href='scambia.php?num_calciatore=$numero&amp;altro_utente=$proprietario' class='user'>scambia</a>";
                if ( $_SESSION[ 'utente' ] != $proprietario and $mercato_libero == "SI" and $stato_mercato == "I" )
                    $offri = "<a href='compra.php?num_calciatore=$numero' class='user'>compra</a>";
                if ( $_SESSION[ 'utente' ] == $proprietario and $mercato_libero == "SI" and $stato_mercato != "I" )
                    $offri = "<a href='cambi.php?num_calciatore=$numero' class='user'>cambi</a>"; elseif ( $_SESSION[ 'utente' ] == $proprietario and $mercato_libero == "SI" and $stato_mercato == "I" )
                    $offri = "<a href='vedi_vendi_subito.php?num_calciatore=$numero' class='user'>vendi ora</a>";

                if ( $chiusura_giornata == 1 )
                    $offri = "<font color='red' size='-2'>Giornata chiusa!</font>";

                if ( $num1 % 2 )
                    $colore = $colore_riga_alt; else $colore = "#FFFFFF";
                ...
                if ( $_SESSION[ 'utente' ] != $proprietario and $mercato_libero != "SI" and $stato_mercato == "B" )
                    $offri = "<a href='busta_offerta.php?num_calciatore=$numero' class='user'>offri</a>"; elseif ( $_SESSION[ 'utente' ] != $proprietario and $mercato_libero != "SI" )
                    $offri = "<a href='offerta.php?num_calciatore=$numero' class='user'>rilancia</a>";
                ...
                if ( $vecchio_proprietario ) {
                    $proprietario_mostra = "$proprietario <font size='-2'>(offerta precedente di $vecchio_proprietario)</font>";
                } else {
                    $proprietario_mostra = $proprietario;
                }
                ...
                if ( $num1 % 2 )
                    $color = $colore_riga_alt; else $color = "#FFFFFF";
    ...
    */
    ...
    $xsquadra_ok = "NO";
    if ( $num_calciatori_comprabili <= 0 or $soldi_spendibili <= 0 ) {
        $offribile = "NO";
    }
    ...
    if ( $max_calciatori != $num_calciatori_posseduti ) {
        ...
        for ( $num1 = 0; $num1 < $num_giocons; $num1++ ) {
            ...
            if ( $composizione_squadra[ $num1 ] == $schema_giocatori ) {
                $xsquadra_ok = "SI";
            }
        } # fine for $num1
        ...
        if ( $xsquadra_ok != "SI" ) {
            ...
            if ( $np == substr( $verifica_sg, 0, 1 ) )
                ...
                $controlla_squadra .= "Portieri: $np su " . substr( $verifica_sg, 0, 1 ) . " - <font color='red'>Vendere esubero</font><br />";
        ...
        if ( @is_file( DATI_DIR . "/editoriale.txt" ) )
            $linee_editoriale = @file( DATI_DIR . "/editoriale.txt" ) ?: []; else $linee_editoriale = [];

        $num_linee_editoriale = count( $linee_editoriale );
        $messaggio_editoriale = "";

        for ( $num1 = 0; $num1 < $num_linee_editoriale; $num1++ )
            $messaggio_editoriale .= $linee_editoriale[ $num1 ];
        ...
        if ( trim( $messaggi[ 2 ] ) != "" )
            echo "<div class='slogan'>" . html_entity_decode( $messaggi[ 2 ] ) . "</div>";
        ...
        if ( $stato_mercato != "I" )
            echo "Rammenta che in questa fase non puoi fare errori di impostazione della formazione, ogni acquisto &egrave; a titolo definitivo, e non puoi annullare eventuali errori. Si consiglia di iniziare gli acquisti solo dopo aver gi&agrave; creato la squadra su carta.<br /><br />";
        ...
        if ( $mercato_libero == "SI" )
            echo "Modalit&agrave; Magic Cup<br /><br />"; elseif ( $mercato_libero == "NO" )
            echo "Modalit&agrave; Asta Pubblica<br /><br />";

        if ( $stato_mercato == "I" )
            echo " <b>Fase iniziale</b><br />"; elseif ( $stato_mercato == "C" )
            echo " Stato mercato: chiuso<br />";
        elseif ( $stato_mercato == "S" )
            echo " Stato mercato: sospeso<br />";
        elseif ( $stato_mercato == "P" )
            echo " Stato mercato: asta perenne<br />";
        elseif ( $stato_mercato == "B" )
            echo " Stato mercato: buste chiuse<br />";
        elseif ( $stato_mercato == "A" )
            echo " Stato mercato: aperto<br />";
        elseif ( $stato_mercato == "R" )
            echo " Stato mercato: riparazione<br />";

        if ( $stato_mercato == "I" or $stato_mercato == "B" or $stato_mercato == "R" )
            echo "<br />Lista calciatori del: " . date( "d-m-Y H:i:s.", filemtime( DATI_DIR . "/calciatori.txt" ) ) . "<br />";
        ...
        if ( $mercato_libero == "NO" and $stato_mercato == "B" )
            echo "<br /><div class='evidenziato'>Il termine per completare le offerte nelle buste &egrave; il <b>$giornobuste</b> alle ore <b>$orabuste</b></div>";

        if ( $mercato_libero == "NO" and $stato_mercato == "I" )
            echo "L'asta ha un periodo di attesa di giorni <b><font color='red'>$aspetta_giorni</font></b>, ore <b><font color='red'>$aspetta_ore</font></b> e minuti <b><font color='red'>$aspetta_minuti</font></b>.<br />";

        echo "<br />La prossima chiusura automatica &egrave; fissata <br />per il giorno <b>$def_giorno $gc - $mc - $ac</b> alle ore <b>$orac : $minc</b>";

        if ( isset($ultgio) && $ultgio >= 1 )
            echo " e siamo alla giornata n. <b>$ultgio</b>.<br /><br />"; else echo ".<br /><br />";
        ...
        if ( isset( $messaggi[ 2 ] ) and trim( $messaggi[ 2 ] ) != "" )
            echo "<caption>Editoriale</caption><tr><td align='left' colspan='2' bgcolor='$sfondo_tab'>" . html_entity_decode( $messaggi[ 2 ] ) . "</td></tr>";

        $dati_squadra = @file( DATI_DIR . "/squadra_" . $_SESSION[ 'utente' ] );
        ...
        if ( $mercato_libero == "SI" )
            echo "Modalit&agrave; Fanta Cup<br />";

        if ( $stato_mercato == "I" )
            echo " <b>FASE INIZIALE</b><br />"; elseif ( $stato_mercato == "P" )
            echo " Stato mercato: asta perenne<br />";
        elseif ( $stato_mercato == "B" )
            echo " Stato mercato: buste chiuse<br />";
        elseif ( $stato_mercato == "C" )
            echo " Stato mercato: chiuso<br />";
        elseif ( $stato_mercato == "S" )
            echo " Stato mercato: sospeso<br />";
        elseif ( $stato_mercato == "A" )
        ...
        if ( $stato_mercato != "I" and $stato_mercato != "B" and $vedi_campetto == "SI" )
            echo "<img src='/fantacampo.php?orientamento_campetto=1&amp;riduci=80&amp;iutente=$outente' alt='La tua squadra in campo' />";

        echo "</td><td valign='top' align='center'>" . $acapo;

        if ( trim( $messaggi[ 8 ] ) != "" )
            echo "<div>" . html_entity_decode( $messaggi[ 8 ] ) . "</div>"; else immagine_casuale( 'sx', 10, 5 );

        if ( $_SESSION[ "permessi" ] >= 4 )
            echo "<br /><a href='tc.php'>t</a><a href='evoluzioni.php'>e</a>";
        echo "<hr style='clear: both' />
		<div><div style='float:left; width:50%; text-align:left'><a href='messaggi.php' class='user'><b>Ultimi 10 messaggi</b></a><br />";
        ultimi10();
        echo "</div><div style='float:left; width:50%; text-align:left'>" . ultime_notizie( 'tutte' ) . "</div></div></td></tr></table>";
    } # fine else
    ...
    for ( $num1 = 1; $num1 < 40; $num1++ ) {
        ...
        if ( ! @is_file( DATI_DIR . "/" . $giornata_controlla . "_" . $_SESSION[ 'torneo' ] . "_" . $_SESSION[ 'serie' ] ) )
    ...
    $file_giornata = [];
    if ( $giornata_ultima )
        $file_giornata = @file( DATI_DIR . "/giornata" . $giornata . "_" . $_SESSION[ 'torneo' ] . "_" . $_SESSION[ 'serie' ] ) ?: [];
    ...
    if ( $voti_esistenti == "SI" ) {
        ...
        if ( $tipo_campionato != "N" ) {
            ...
            for ( $num1 = $g_inizio_campionato; $num1 < $num_giornata; $num1++ ) {
                ...
                if ( @is_file( DATI_DIR . "/" . $giornata_punti . "_" . $_SESSION[ 'torneo' ] . "_" . $_SESSION[ 'serie' ] ) ) {
                    $file_giornata_p = @file( DATI_DIR . "/" . $giornata_punti . "_" . $_SESSION[ 'torneo' ] . "_" . $_SESSION[ 'serie' ] );
            ...
            while ( list ( $key, $val ) = each( $punti ) ) {
                $posclas++;
                if ( $nome_squadra_memo[ $key ] != "" )
                    echo "$posclas)&nbsp;" . $nome_squadra_memo[ $key ] . ": $val<br/>"; else $posclas = $posclas - 1;
            } # fine while
