...
require_once INC_DIR . "/regexml.inc.php";

$xml_c = new RegEXml;
$fd = $xml_c->get_file( "https://www.gazzetta.it/ssi/2011/boxes/calcio/squadre/" . strtolower( $vedi_squadra ) . "/formazione/formazione.xml" );
...
if ( $ultima_giornata != "00" ) {
    ...
    $cerca_squadra = file( DATI_DIR . "/calciatori.txt" );
...
for ( $num1 = 0; $num1 < $num_cer_squ; $num1++ ) {
    ...
    if ( $vedi_squadra == $xsquadra and $attivo == "1" ) {
        ...
        for ( $num2 = 0; $num2 < $num_calciatori; $num2++ ) {
            ...
            if ( $proprietario == $outente and $numero_calciatore == $numero )
                $sx = "off";
        }
        if ( $ruolo == "P" )
            $color = "white";
        if ( $ruolo == "D" )
            $color = $colore_riga_alt;
        if ( $ruolo == "C" )
            $color = "white";
        if ( $ruolo == "A" )
            $color = $colore_riga_alt;
        ...
        $ruolo_guarda = $ruolo_guarda ?? '';
        $escludi_controllo = $escludi_controllo ?? '';
        ...
        if ( $_SESSION[ 'valido' ] == "SI" and $mercato_libero == "SI" and $stato_mercato == "I" and $sx != "off" )
            $table_layout .= "<td align='center'><a href='compra.php?num_calciatore=$numero&amp;valutazione=$valutazione' class='user'>compra</a></td>"; elseif ( $_SESSION[ 'valido' ] == "SI" and $mercato_libero == "SI" and $stato_mercato != "I" and $sx != "off" )
            $table_layout .= "<td align='center'><a href='cambi.php?num_calciatore=$numero' class='user'>cambia</a></td>";
        elseif ( $_SESSION[ 'valido' ] == "SI" and $mercato_libero == "NO" and $stato_mercato == "I" and $sx != "off" )
            $table_layout .= "<td align='center'><a href='offerta.php?num_calciatore=$numero' class='user'>offri</a></td>";
        ...
        elseif ( $_SESSION[ 'valido' ] == "SI" and $mercato_libero == "NO" and $stato_mercato == "B" and $sx != "off" )
            $table_layout .= "<td align='center'><a class='user'>inserisci nella busta</a></td>";
        elseif ( $_SESSION[ 'valido' ] == "SI" and $mercato_libero == "NO" and $stato_mercato == "P" and $sx != "off" )
            $table_layout .= "<td align='center'><a href='offerta.php?num_calciatore=$numero' class='user'>offri</a></td>";
        elseif ( $_SESSION[ 'valido' ] == "SI" and $mercato_libero == "NO" and $stato_mercato == "S" and $sx != "off" )
            $table_layout .= "<td align='center'><a href='offerta.php?num_calciatore=$numero' class='user'>offri</a></td>";
        elseif ( $_SESSION[ 'valido' ] == "SI" and $mercato_libero == "NO" and $stato_mercato == "A" and $sx != "off" )
            $table_layout .= "<td align=center><a href='scambia.php?num_calciatore=$numero&amp;altro_utente=$proprietario' class='user'>scambia</a></td>";
...
<table summary="Dati squadra" width="100%" style="background-color: white; text-align: center; background-image:url('http://images.gazzettaobjects.it/libs/css/default_theme/assets/bg_squadre/<?php echo strtolower( $vedi_squadra ) ?>.gif');background-repeat:no-repeat;">
        <tr>
            <td style="width: 50%; padding: 10px; text-align: justify; vertical-align: top">
                <br/><br/><br/>
<?php
$grab1 = file( "https://www.gazzetta.it/calcio/squadre/" . strtolower( $vedi_squadra ) . '/notizie/' );
for ( $nn = 1900; $nn <= 2200; $nn++ ) {
    if ( isset( $grab1[ $nn ] ) && strstr( $grab1[ $nn ], "sede-data bg1" ) ) {
        $lintit = $nn + 2;
        $lingn = $nn + 3;
        if ( strlen( strip_tags( $grab1[ $lingn ] ) ) > 50 ) {
            $gn = utf8_encode( html_entity_decode( $grab1[ $lingn ] ) );
            $tit = utf8_encode( html_entity_decode( $grab1[ $lintit ] ) );
        }
    }
}
...
if ( @is_file( $file1 ) )
    echo "<img src='$file1' style='padding: 10px; margin: 0px; float: left;' alt='Stemma sociale' />";
...
if ( $_SESSION[ 'valido' ] != "SI" ) {
	...
	<div id='destra'>";
    require_once PAGES_DIR . "/menu_i.php";
    echo "</div>";
}
