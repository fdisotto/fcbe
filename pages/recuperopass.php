...
require_once INC_DIR . "/rec/sendmail_include.php";
...
if ( isset( $_GET[ 'email' ] ) && isset( $_GET[ 'account' ] ) ) {
    ...
    for ( $num1 = 1; $num1 < $num_tornei; $num1++ ) {
        ...
        if ( is_file( DATI_DIR . "/utenti_" . $otid . ".php" ) ) {
            $file = @file( DATI_DIR . "/utenti_" . $otid . ".php" ) or die( "Impossibile caricare i dattagli degli utenti [RIF: ./dati/utenti $otid]" );
            $linee = count( $file );
            $linea = 1;
            $error = 0;
    ...
    if ( ! preg_match( "/^[0-9a-z]{32}$/", $_GET[ 'account' ] ) ) {
        $error = 1;
    }
    if ( $error == 0 ) {
        ...
        $percorso_file = DATI_DIR . "/utenti_" . $otorneo . ".php";
        //echo $percorso_file;
        ...
        require_once DATI_DIR . "/menu_i.php";
        echo "</div></div>";
        die();
    } else {
        $error = 1;
...
if ( isset( $error ) && $error == 1 ) {
    ...
    if ( ( $_SESSION[ 'CONTROLLO' ] == $_POST[ 'security_code' ] ) && ( ! empty( $_SESSION[ 'CONTROLLO' ] ) ) ) {
        ...
        echo "<tr><td width ='25%'><div align='center'><font text='red'>Codice Immagine Errato </font><br /><br />\n<a href='recuperopass.php'>Riprova</a></div></td></tr>";
    }
} else {
    ...
    ?>
    <tr>
        <td align=center width='100%'>Codice di Controllo</td>
    </tr>
    <tr>
        <td align=center width='100%'><img src="/captcha.inc.php"></td>
    </tr>
    <tr>
        <td align=center width='100%'><input id="security_code" name="security_code" type="text"/></td>
    </tr>
    <?php echo "<tr><td align=center width = '100%'><input type='submit' name='send_password' value='Invia Password'  /></td></tr>\n";
    echo "</form>\n</div>\n";
}
echo "</tr>
</table>
