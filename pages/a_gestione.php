...
if ( $_SESSION[ 'valido' ] == "SI" and $_SESSION[ 'permessi' ] == 5 ) {
    ...
    if ( isset( $ccfv ) && $ccfv == "SI" && isset( $clfv ) ) {
        ...
        file_put_contents( DATI_DIR . "/calciatori.txt", $calciatori_remote, LOCK_EX );
        $errori = error_get_last();
        if ( ! empty( $errori ) ) {
    ...
    if ( isset( $blocca_giornata ) && $blocca_giornata == "chiudi" ) {
        file_put_contents( DATI_DIR . "/chiusura_giornata.txt", "1", LOCK_EX );
    ...
    if ( isset( $blocca_giornata ) && $blocca_giornata == "apri" ) {
        if ( @is_file( DATI_DIR . "/chiusura_giornata.txt" ) ) {
            unlink( DATI_DIR . "/chiusura_giornata.txt" );
    ...
    if ( isset( $cambia_data ) && $cambia_data == "cambia_data" ) {
        file_put_contents( DATI_DIR . "/data_chigio.txt", "$annom$mesem$giornom$oram$minutim", LOCK_EX );
    ...
    if ( is_file( DATI_DIR . "/data_chigio.txt" ) )
        $data_chigio = @file( DATI_DIR . "/data_chigio.txt" );
    ...
    while ( $num1 <= 38 ) {
        ...
        for ( $num0 = 1; $num0 < $num_tornei; $num0++ ) {
            if ( @is_file( DATI_DIR . "/" . $giornata . "_" . $num0 . "_0" ) and $num1 >= $prossima ) {
    ...
    if ( $clfv == "NO" and $lfv == "NO" ) {
        $tabella_giornate .= "<div style='float: center; padding: 22px;'><b>Procedura disattivata da pannello config!</b>";
    } else {
        $tabella_giornate .= "<div style='float: left; padding: 5px;'><form method='post' action='/a_gestione.php'>
    ...
    require_once INC_DIR . '/flount.php';
