...
if ( isset( $_SESSION[ 'valido' ] ) && $_SESSION[ 'valido' ] != "SI" and $_SESSION[ 'ec' ] != "SI" ) {
    echo "<meta http-equiv='refresh' content='0; url=logout.php?logout=2'>";
    exit;
} elseif ( isset( $_SESSION[ 'valido' ] ) && $_SESSION[ 'valido' ] == "SI" && isset( $_SESSION[ 'csn' ] ) && $_SESSION[ 'csn' ] == $_SERVER[ 'SERVER_NAME' ] ) {
    if ( $_SESSION[ 'utente' ] == $admin_user and $_SESSION[ 'pass' ] == $admin_pass ) {
    } elseif ( isset( $_SESSION[ "admin" ] ) && $_SESSION[ "admin" ] == "SI" ) {
        ...
        $fileo = @file( DATI_DIR . "/utenti_" . $_SESSION[ "torneo" ] . ".php" ) or die( "Impossibile caricare i dattagli degli utenti [RIF: ./dati/utenti.php](" . $_SESSION[ "torneo" ] . ")" );
...
} elseif ( isset( $_SESSION[ 'ec' ] ) && $_SESSION[ 'ec' ] != "SI" ) {
    pr( $_SESSION );
    pr( $_SERVER );
...
if ( isset( $_SESSION[ 'utente' ] ) && $_SESSION[ 'utente' ] != $admin_user && isset( $_SESSION[ 'valido' ] ) && $_SESSION[ 'valido' ] == "SI" ) {
    ...
    for ( $num1 = 1; $num1 < $linee_tornei; $num1++ ) {
        ...
        if ( $_SESSION[ "torneo" ] == $otid )
            break;
    } # fine for $num$tornei = @file("DATI_DIR/tornei.php");
    ...
    $numero_cambi_max = $otnumero_cambi_max;                # in mercato libero � il massimo dei cambi consentiti
    ...
    $max_in_panchina = $otmax_in_panchina;                    # Numero di calciatori in panchina e quanti ne possono entrare. Si possono fare sostituzioni per ruolo (il calciatore entra se un'altro del suo ruolo non ha giocato) o per schema (il calciatore entra se entrando lo schema che si forma � tra quelli consentiti). Se sia per ruolo che per schema sono a SI si sostituisce prima per ruolo.
    ...
    # Dati per i campionati a scontri diretti. Servono solo se si � impostato un campionato a "S".
    ...
    if ( is_file( DATI_DIR . "/chiusura_giornata.txt" ) )
        $chiusura_giornata = INTVAL( @file( DATI_DIR . "/chiusura_giornata.txt" ) );

    if ( is_file( DATI_DIR . "/data_chigio.txt" ) )
        $data_chigio = @file( DATI_DIR . "/data_chigio.txt" ); else $data_chigio[ 0 ] = $data_attuale;
    ...
    if ( $giorno_chiusura == 6 )
        $def_giorno = "sabato"; elseif ( $giorno_chiusura == 5 )
        $def_giorno = "venerd&igrave;";
    elseif ( $giorno_chiusura == 4 )
        $def_giorno = "gioved&igrave;";
    elseif ( $giorno_chiusura == 3 )
        $def_giorno = "mercoled&igrave;";
    elseif ( $giorno_chiusura == 2 )
        $def_giorno = "marted&igrave;";
    elseif ( $giorno_chiusura == 1 )
        $def_giorno = "luned&igrave;";
    elseif ( $giorno_chiusura == 0 )
        $def_giorno = "domenica";

    if ( $data_attuale > $verdatachiusura )
        $mextex = "<font color='red'>Data attuale maggiore alla data di chiusura</font><br/>"; elseif ( $data_attuale < $verdatachiusura )
        $mextex = "Data attuale minore alla data di chiusura";
    elseif ( $data_attuale = $verdatachiusura )
        $mextex = "Data attuale uguale alla data di chiusura";

    if ( $chiusura_giornata == 0 )
        $mess02 = "E' possibile modificare la formazione o cambiare i calciatori al prossimo <b>$def_giorno</b> entro le ore <b>$orac</b> e <b>$minc</b> minuti."; else $mess02 = "<b><font color=red>Giornata chiusa!</font></b><br/>Non &egrave; possibile modificare la formazione o effettuare azioni di mercato. Attendere l'aggiornamento delle quotazioni.<br/>";
    ...
    if ( $data_attuale >= $verdatachiusura and $chiusura_giornata != 1 ) {
        ...
        $file_dati = fopen( DATI_DIR . "/chiusura_giornata.txt", "wb" );
