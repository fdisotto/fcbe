...
if ( $_SESSION[ 'valido' ] == "SI" and ( $stato_mercato != "I" or $stato_mercato != "R" or $_SESSION[ 'permessi' ] == 4 ) ) {
    ...
    if ( $chiusura_giornata >= 1 ) {
        $nome_squadra = "tutti";
        ############################################
        $utenti = @file( DATI_DIR . "/utenti_" . $_SESSION[ 'torneo' ] . ".php" );
        $linee = count( $utenti );
        for ( $num1 = 1; $num1 < $linee; $num1++ ) {
            ...
            if ( $ourl and $ourl != "http://" )
                $titolo .= "<br />Telefono: <b>$ourl</b>";
            if ( file_exists( DATI_DIR . "/squadra_" . $outente ) ) {
                $titolo .= "<br />Ultima modifica formazione: " . date( "d-m-Y H:i:s", filemtime( DATI_DIR . "/squadra_$outente" ) );
            } else $titolo .= "<br />Ultima modifica formazione: Nessuna formazione schierata";
            $cambi_effettuati = INTVAL( $ocambi );
            $cambi_total = ( $numero_cambi_max ?? 0 ) + ( $cambi_extra ?? 0 );
	...
	<tr><td align='center' colspan='$colspan'><b>Titolari</b></td></tr>";
            ...
            if ( ! $home_page_squadra )
                $home_page_squadra_vedi = "http://";
            ...
            if ( $outente == $_SESSION[ 'utente' ] ) {
                for ( $num2 = "01"; $num2 < 40; $num2++ ) {
                    if ( strlen( $num2 ) == 1 )
                        $num2 = "0" . $num2;
                    $giornata = DATI_DIR . "/giornata$num2";
                    if ( ! @is_file( $giornata ) )
                        break;
                } # fine for $num2
	...
	<caption>$titolo</caption><tr><td valign='top'>";
            ...
            if ( $tab_lato != "" and $xsquadra_ok != "SI" )
                echo "$tab_lato<hr /><br/>FantaEuro residui: <b>$soldi_spendibili</b> Fanta-Euro.<br/><br/>
