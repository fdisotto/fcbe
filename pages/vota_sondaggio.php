...
if ( $_SESSION[ 'valido' ] == "SI" ) {
    ...
    if ( @is_file( DATI_DIR . "/sondaggio.php" ) ) {
        ...
        include( DATI_DIR . "/sondaggio.php" );
        $voti_giocatore = "voti_" . $_SESSION[ 'utente' ];
        $voti_giocatore = count( $$voti_giocatore );

        if ( $voti_consentiti != 0 and $voti_giocatore >= $voti_consentiti )
            echo "<b>Hai raggiunto il limite massivo di votazioni!</b><br/><br/>"; else echo "Sono consentiti <b>$voti_consentiti</b> voti.<br/><br/>";

        if ( isset( $vota ) && isset($voto) && ( $voti_consentiti == 0 || $voti_giocatore < $voti_consentiti ) ) {
            require_once DATI_DIR . "/sondaggio.php";
            $linee_sondaggio = file( DATI_DIR . "/sondaggio.php" );
            $filesondaggio = fopen( DATI_DIR . "/sondaggio.php", "w+" );
            ...
            for ( $num1 = 0; $num1 < $num_linee_sondaggio; $num1++ ) {
                if ( substr( $linee_sondaggio[ $num1 ], 0, 6 ) == "\$voti" . $voto or substr( $linee_sondaggio[ $num1 ], 0, 7 ) == "\$voti" . $voto ) {
                    $linee_sondaggio[ $num1 ] = "\$voti" . $voto . " = " . $$voti . ";";
                ...
                if ( $voti_consentiti != 0 or $voto_palese == "SI" ) {
                    if ( substr( $linee_sondaggio[ $num1 ], 0, $lung_nome_utente ) == "\$voti_" . $_SESSION[ 'utente' ] ) {
                        ...
                        if ( $voto_palese == "NO" )
                            $nuovo_voto = 0; else $nuovo_voto = $voto;
            ...
            require( DATI_DIR . "/sondaggio.php" );
        ...
        if ( $voto_palese == "SI" ) {
            ...
            for ( $i = 1; $i < sizeof( $file ); $i++ ) {
                ...
                if ( $outente != "admin" ) {
                    ...
                    $lista_voti = "";
                    for ( $num2 = 0; $num2 < $num_voti_giocatore; $num2++ )
                        $lista_voti .= ${$voti_giocatore}[ $num2 ] . ", ";
