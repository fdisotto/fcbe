...
if ( $_SESSION[ 'utente' ] == $_GET[ 'iutente' ] ) {
    ...
    if ( $id_gioc == "" and $controllo_id_gioc == 1 ) {
        $dati_squadra = @file( DATI_DIR . "/squadra_$iutente" );
        if ( isset( $dati_squadra[ 1 ] ) )
            $titolari = explode( ",", $dati_squadra[ 1 ] ); else $titolari = array();

        if ( isset( $test ) ) {
            echo "<b>Modalita senza parametro id_gioc.</b><br />";
            if ( is_file( DATI_DIR . "/squadra_$iutente" ) ) {
                echo "Uso file: DATI_DIR/squadra_$iutente<br />";
                if ( isset( $dati_squadra[ 1 ] ) )
                    echo "Dati titolari: " . $dati_squadra[ 1 ] . "<br />totale titolari: <b>" . count( $titolari ) . "</b><br />"; else echo "Dati titolari mancanti<br />";
                if ( isset( $dati_squadra[ 2 ] ) )
                    echo "Dati panchinari: " . $dati_squadra[ 2 ] . "<br />"; else echo "Dati panchinari mancanti<br />";
            } else echo "Non trovato file: DATI_DIR/squadra_$iutente<br />";
        }
    } else {
    ...
    for ( $num1 = 0; $num1 <= $num_titolari; $num1++ ) {
        ...
        if ( $numero_titolare >= 100 and $numero_titolare <= 199 ) {
            ...
            $cd++;
            $titolari_1[] = $numero_titolare;
        } elseif ( $numero_titolare >= 500 and $numero_titolare <= 699 ) {
            $cc++;
            $titolari_1[] = $numero_titolare;
        } elseif ( $numero_titolare >= 700 and $numero_titolare <= 999 ) {
            $ca++;
            $titolari_1[] = $numero_titolare;
        }
    } # fine for $num1
    ...
    if ( @$ultima_giornata >= 1 and file_exists( $percorso_cartella_voti . "/voti$ultima_giornata.txt" ) ) {
        ...
        $cerca_valutazione = @file( DATI_DIR . "/calciatori.txt" );
        sort( $cerca_valutazione, SORT_NUMERIC );
        if ( isset( $test ) ) {
            if ( is_file( DATI_DIR . "/calciatori.txt" ) )
                echo "Uso file: DATI_DIR/calciatori.txt<br />"; else echo "Non trovato file: DATI_DIR/calciatori.txt<br />";
        }
    }
        ...
        if ( $riga_dati_calciatori != -1 ) {
            ...
            if ( $contatore <= 11 ) {
                ...
                if ( isset( $test ) ) {
                    if ( is_file( "$m_squadra" ) )
                        echo "$contatore Esiste file: $m_squadra per il giocatore " . $titolari[ $contatore - 1 ] . " $mnome $mnome1 <img src='$m_squadra' /><br />"; else echo "$contatore Non esiste file: $m_squadra MA GESTITO!  per il giocatore " . $titolari[ $contatore - 1 ] . " $mnome $mnome1<br />";
                }

                if ( isset( $test ) and ! is_file( "$m_squadra" ) ) {
                    if ( is_file( "./immagini/m_" . "no_squadra" . $m_estenzione ) )
                        echo "$contatore Esiste file: ./immagini/m_no_squadra" . $m_estenzione . " per il giocatore " . $titolari[ $contatore - 1 ] . " $mnome $mnome1 <img src='./immagini/m_no_squadra" . $m_estenzione . "' /><br />"; else echo "$contatore Non esiste file: ./immagini/m_no_squadra" . $m_estenzione . " per il giocatore " . $titolari[ $contatore - 1 ] . " $mnome $mnome1<br />";
                }
                ...
                if ( $gd_support == "GIF" )
                    $imm_calciatore = ImageCreateFromGif( "$m_squadra" ); else $imm_calciatore = ImageCreateFromPng( "$m_squadra" );
                ...
                if ( $orientamento_campetto == 0 ) {
                    ...
                    if ( $mruolo == "0" ) {
                        $altezza = $alt_p;
                        $larghezza = ( $lar_imm1 - $w ) / 2;
                    } elseif ( $mruolo == "1" ) {
                        $larghezza = $lar_imm1 - ( $w * $cd );
                        $larghezza = $larghezza / ( $cd + 1 );
                        $larghezza = ( $larghezza + $w ) * $spazio_d - $w;
                        $altezza = ( $alt_imm1 - $h ) / 4 + $alt_p;
                        $spazio_d++;
                    } elseif ( $mruolo == "2" ) {
                        $larghezza = $lar_imm1 - ( $w * $cc );
                        $larghezza = $larghezza / ( $cc + 1 );
                        $larghezza = ( $larghezza + $w ) * $spazio_c - $w;
                        $altezza = ( $alt_imm1 - $h ) * 2 / 4 + $alt_p;
                        $spazio_c++;
                    } elseif ( $mruolo == "3" ) {
                        $larghezza = $lar_imm1 - ( $w * $ca );
                        $larghezza = $larghezza / ( $ca + 1 );
                        $larghezza = ( $larghezza + $w ) * $spazio_a - $w;
                        $altezza = ( $alt_imm1 - $h ) * 3 / 4 + $alt_p;
                        $spazio_a++;
                    }
                    ...
                    imagefill($output,0,0,0x7fff0000);
                    ImageCopyResampled($output, $imm_calciatore, 0, 0, 0, 0, $lar_imm2 * $coefficient, $alt_imm2 * $coefficient, $lar_imm2, $alt_imm2);


                    ImageCopy( $campo, $output, $larghezza, $altezza, 0, 0, $w, $h );
                    $spaziatura = 4;
                    $altezza_testo = $altezza + $h + $spaziatura;
                    ...
                    if ( $mruolo == "0" ) {
                        $larghezza = $alt_p;
                        $altezza = ( $alt_imm1 - $h ) / 2;
                    } elseif ( $mruolo == "1" ) {
                        $altezza = $alt_imm1 - ( $h * $cd );
                        $altezza = $altezza / ( $cd + 1 );
                        $altezza = ( $altezza + $h ) * $spazio_d - $h;
                        $larghezza = ( $lar_imm1 - $w ) / 4 + $alt_p;
                        $spazio_d++;
                    } elseif ( $mruolo == "2" ) {
                        $altezza = $alt_imm1 - ( $h * $cc );
                        $altezza = $altezza / ( $cc + 1 );
                        $altezza = ( $altezza + $h ) * $spazio_c - $h;
                        $larghezza = ( $lar_imm1 - $w ) * 2 / 4 + $alt_p;
                        $spazio_c++;
                    } elseif ( $mruolo == "3" ) {
                        $altezza = $alt_imm1 - ( $h * $ca );
                        $altezza = $altezza / ( $ca + 1 );
                        $altezza = ( $altezza + $h ) * $spazio_a - $h;
                        $larghezza = ( $lar_imm1 - $w ) * 3 / 4 + $alt_p;
                        $spazio_a++;
                    }
            ...
            if ( isset( $test ) ) {
                if ( is_file( "$m_squadra_no" ) )
                    echo "$contatore Esiste file $m_squadra_no per il giocatore " . $titolari[ $contatore - 1 ] . " <img src='$m_squadra_no' /><br />"; else echo "$contatore Non esiste file $m_squadra_no  per il giocatore  " . $titolari[ $contatore - 1 ] . "<br />";
            }

            if ( $gd_support == "GIF" )
                $imm_calciatore = ImageCreateFromGif( "$m_squadra_no" ); else $imm_calciatore = ImageCreateFromPng( "$m_squadra_no" );
            //
            $lar_imm2 = imagesx( $imm_calciatore );
            $alt_imm2 = imagesy( $imm_calciatore );
            ...
            if ( $orientamento_campetto == 0 ) {
                ...
                ImageCopy( $campo, $output, $larghezza, $altezza, 0, 0, $lar_imm2, $alt_imm2 );
                $spaziatura = 4;
                $altezza_testo = $altezza + $h + $spaziatura;
    ...
    if ( $ultima_giornata != "" ) {
        $riga_testo += 10;
        ImageString( $campo, 1, 15, $riga_testo, "Giornata: $ultima_giornata", $colore_testo );
    }
    ...
    if ( isset( $test ) ) {
        ...
        echo "<br /><br />";
        echo "1 SERVER_NAME.PHP_SELF: http://" . $_SERVER[ 'HTTP_HOST' ] . @$_SERVER[ 'PHP_SELF' ] . "<br />";
        echo "2 urlsito: " . $_SERVER[ 'HTTP_HOST' ] . "/fantacampo.php<br />";
	...
	test: $test<br />";
        ...
        if ( file_exists( "./immagini/campo_r.jpg" ) )
            echo "Esiste il file ./immagini/campo_r.jpg<br /><img src='./immagini/campo_r.jpg' /><br />"; else echo "Non esiste il file ./immagini/campo_r.jpg<br />";
        if ( file_exists( "./immagini/campo.jpg" ) )
            echo "Esiste il file ./immagini/campo.jpg<br /><img src='./immagini/campo.jpg' /><br />"; else echo "Non esiste il file ./immagini/campo.jpg<br />";
        echo "<br />ULTERIORI VERIFICHE SQUADRE-IMMAGINI:<br />";
        if ( file_exists( DATI_DIR . "/squadre.txt" ) )
            echo "Esiste il file DATI_DIR/squadre.txt<br /><br />"; else echo "Non esiste il file DATI_DIR/squadre.txt<br />";
        ...
        for ( $num1 = 0; $num1 < $numteam; $num1++ ) {
            ...
            if ( file_exists( $file_input_imgm ) )
                echo "Esiste il file $file_input_imgm <img src='$file_input_imgm' /><br />"; else echo "Non esiste il file $file_input_imgm <br />";
        }
        $msquadra = "no_squadra";
        ...
        if ( file_exists( $file_input_imgm ) )
            echo "Esiste il file $file_input_imgm <img src='$file_input_imgm' /><br />"; else echo "Non esiste il file $file_input_imgm <br />";
        echo "<br /><br />";
        echo "<h2>Prova funzionamento</h2><img src='http://" . $_SERVER[ 'HTTP_HOST' ] . @$_SERVER[ 'PHP_SELF' ] . "?iutente=$iutente&orientamento_campetto=$orientamento_campetto&riduci=$riduci&riduci1=$riduci1' border='0' align='center' alt='Squadra in campo $nome_squadra di $iutente' title='Squadra in campo $nome_squadra di $iutente' />";
        echo "<br /><br />";
    }#end if(isset($test))
}
