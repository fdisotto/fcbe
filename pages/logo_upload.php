...
if ( $_SESSION[ 'valido' ] == "SI" and $_SESSION[ 'permessi' ] >= 0 ) {
    ...
    if ( ! is_writable( $uploadpath ) )
        echo "La cartella $uploadpath non &egrave; scrivibile!<br /> Impostare CHMOD 775.<br />";
    if ( ! is_readable( $uploadpath ) )
        echo "La cartella $uploadpath non &egrave; leggibile!<br /> Impostare CHMOD 775.<br />";
    ...
    if ( isset( $_FILES[ 'fileup' ] ) && strlen( $_FILES[ 'fileup' ][ 'name' ] ) > 1 ) {
        ...
        if ( ! in_array( $type, $allowtype ) )
            $err .= 'Il file <b>' . $_FILES[ 'fileup' ][ 'name' ] . '</b> ha un estensione non valida';
        if ( $_FILES[ 'fileup' ][ 'size' ] > $max_size * 1000 )
            $err .= '<br/>La dimensione massima del file &egrave;: ' . $max_size . ' KB.';
        if ( isset( $width ) && isset( $height ) && ( $width >= $alwidth || $height >= $alheight ) )
            $err .= '<br/>Il file deve essere massimo: ' . $alwidth . ' x ' . $alheight;

        // If no errors, upload the image, else, output the errors
        if ( $err == '' ) {
            if ( file_exists( $_SESSION[ 'utente' ] . ".jpg" ) )
                unlink( $_SESSION[ 'utente' ] . ".jpg" );
