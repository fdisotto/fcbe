...
echo "<br/><center><h3>Calendario scontri diretti</h3>".$acapo;
echo "<div align='center'>$vedi_giornate</div><br/><br/>".$acapo;
...
for($num1 = 0 ; $num1 < $num_campionati; $num1++) {
	...
	if ($tipo_campionato != "N") {
		echo "<b>Campionato</b> dalla giornata <b>$g_inizio_camp</b> alla <b>$g_fine_camp</b>".$acapo;
		if ($tipo_campionato == "V") echo ": classifica per somma voti.<br/><br/>".$acapo;
		if ($tipo_campionato == "P") echo ": classifica a punti per posizione.<br/><br/>".$acapo;

		if ($tipo_campionato == "S") {
			echo ": scontri diretti.<br/><br/>";
			$attiva_scontri_diretti = "NO";
			$contaUtenti = 0;

			for($linea = 1; $linea < $linee; $linea++){
				@list($ooutente, $oopass, $oopermessi, $ooemail, $oourl, $oosquadra, $ootorneo, $ooserie, $oocitt�, $oocrediti, $oovariazioni, $oocambi, $ooreg) = explode("<del>", $file_utenti[$linea]);
				$contaUtenti++;
			}
