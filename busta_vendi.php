...
require ("./header.php");
	...
	echo "<br><br><table bgcolor=\"$sfondo_tab\" align=center cellpadding=20><tr><td><center><h3>Svincolo calciatore</h3>";
	$chiusura_giornata = (INT) @file("$percorso_cartella_dati/chiusura_giornata.txt");
		...
		echo "<br><center><h3>Giocatore non trovato<br>Non puoi vendere questo calciatore</h3></center><br><br><br><br><br><br><br>";
			...
			if ($mercato_libero == "SI") {
				...
				<form method=\"post\" action=\"busta_vendi_subito.php\">
			...
			#	$frase1 = "Lo svincolo &egrave; effettuato tenendo presente il $percentuale_vendita % del costo di acquisto.";
				$azione1 = "<center><b>ELIMINARE IL CALCIATORE DALLA BUSTA</b></center></br>
				<center>Questa operazione consente di togliere il calciatore <b>$nome</b> della busta chiusa per sostituirlo con un'altro disponibile.<br><br><font color=red><b>Procedere di seguito all'acquisto di un'altro calciatore.</b></font><br>
				<form method=\"post\" action=\"busta_vendi_subito.php\">
				<input type=\"hidden\" name=\"prezzo_vendita\" value=\"$prezzo_vendita\">
				<input type=\"hidden\" name=\"num_calciatore\" value=\"$num_calciatore\">
				<input type=\"submit\" name=\"vendi_subito\" value=\"Togli subito\">
				 il calciatore <b>$nome</b> dalla busta chiusa.
				</form></center>";
				}
				...
				<br><br><center><form method=\"post\" action=\"busta_vendi_subito.php\">
				<input type=\"hidden\" name=\"num_calciatore\" value=\"$num_calciatore\">
		...
		echo "<table border=1 class=border cellpadding=10 cellspacing=10 align=center bgcolor=\"$sfondo_tab\"><tr><td>$frase1</td></tr><tr><td>$azione1</td></tr></table>";
		} # fine else ($trovato == "NO")
	} # fine if ($Chiusura_giornata != 1)

	else {
	echo "<center><h2>Giornata chiusa</h2></center>";
	echo "<p align=center>Non &egrave; più consentito effettuare operazioni per questa giornata!<br><br>Attendere fino a quando viene creata la prossima giornata.</p>";
	}
