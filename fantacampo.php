...
if (!isset($test)) header("Content-Type: image/jpeg;");
if (isset($test)) echo "<h2>MODALITA FANTACAMPO CHE FA IL TEST DEGLI OGGETTI</h2><br />";

$controllo_id_gioc=0;
if (isset($_GET['iutente'])) {
	$iutente = @$_GET['iutente'];
	if (isset($test)) echo "iutente: $iutente preso dal parametro<br />";
}
else {
	$iutente = @$_SESSION['utente'];
	if (isset($test)) echo "iutente: $iutente preso dalla sessione<br />";
}
#
if (isset($_GET['id_gioc'])){
	$id_gioc = @$_GET['id_gioc'];
	if (isset($test)) echo "id_gioc: $id_gioc preso dal parametro<br />";
}
else {
	$id_gioc = "";
	$controllo_id_gioc = 1;
	if (isset($test)) echo "Parametro id_gioc non presente<br />";
}
...
if ($riduci < 40) $riduci = 40;
if ($riduci > 200) $riduci = 200;
...
{
	$campo2 = ImageCreateFromJpeg("./immagini/campo.jpg");
}
...
if (isset($test)) echo "controllo_id_gioc $controllo_id_gioc<br />";

if ($id_gioc == "" and $controllo_id_gioc == 1)
{$dati_squadra = @file($percorso_cartella_dati."/squadra_$iutente");
if (isset($dati_squadra[1])) $titolari = explode(",", $dati_squadra[1]);
else $titolari = array();
...
if (isset($test)) {
	...
	if (is_file($percorso_cartella_dati."/squadra_$iutente")) {
		...
		if (isset($dati_squadra[1])) echo "Dati titolari: ".$dati_squadra[1]."<br />totale titolari: <b>".count($titolari)."</b><br />";
		else echo "Dati titolari mancanti<br />";
		if (isset($dati_squadra[2]))echo "Dati panchinari: ".$dati_squadra[2]."<br />";
		else echo "Dati panchinari mancanti<br />";
	}
	else echo "Non trovato file: $percorso_cartella_dati/squadra_$iutente<br />";
}
}
else
{$titolari = explode(",", $id_gioc);
if (isset($test)) echo "<b>Modalita parametro id_gioc:  $id_gioc</b><br />";
}
...
for ($num1 = 0 ; $num1 <= $num_titolari; $num1++) {
	...
	{ if ($cp == 0) {$cp++; $titolari_1[]=$numero_titolare;}
	elseif (isset($test)) echo "C'e' piu' di un portiere! <b>($numero_titolare)</b> <br />";
}
elseif ($numero_titolare >= 200 and $numero_titolare <= 499) { $cd++; $titolari_1[]=$numero_titolare;}
elseif ($numero_titolare >= 500 and $numero_titolare <= 799) { $cc++; $titolari_1[]=$numero_titolare;}
elseif ($numero_titolare >= 800 and $numero_titolare <= 999) { $ca++; $titolari_1[]=$numero_titolare;}
} # fine for $num1
unset($num1);

if (isset($test) and ($cp+$cd+$cc+$ca) > 11 ) echo "Ci sono piu' di 11 giocatori! <br />";
...
if (@$ultima_giornata == "" ) $ultima_giornata = ultima_giornata_giocata();
...
if (@$ultima_giornata >= 1 and file_exists($percorso_cartella_voti."/voti$ultima_giornata.txt") ){
	...
	if (isset($test)) echo "Uso file: $percorso_cartella_voti/voti$ultima_giornata.txt<br />";
}
else
{ $cerca_valutazione = @file("$percorso_cartella_dati/calciatori.txt");
sort($cerca_valutazione,SORT_NUMERIC);
if (isset($test)) {
	if (is_file("$percorso_cartella_dati/calciatori.txt")) echo "Uso file: $percorso_cartella_dati/calciatori.txt<br />";
	else echo "Non trovato file: $percorso_cartella_dati/calciatori.txt<br />";
}
}

$num_calciatori = count($cerca_valutazione);
if (isset($test)) echo "Righe <b>( $num_calciatori )</b> dati alla riceca del nome calciatore e maglia e ruolo<br />";
$contatore = 1; $spazio_d = 1; $spazio_c = 1; $spazio_a = 1;
...
for ($num1 = 0 ; $num1 <= $num_titolari; $num1++) {
	...
	if ($riga_dati_calciatori != -1 ) {
		...
		if ($mnumero == $numero_titolare and $contatore <= 11 ) {
			...
			# i luigi
			$m_squadra = "./immagini/t_".strtolower($msquadra).".png";
			#$m_squadra = "./immagini/m_".strtolower($msquadra).$m_estenzione;
			# f luigi

			if (isset($test)){
				if (is_file("$m_squadra")) echo "$contatore Esiste file: $m_squadra per il giocatore ".$titolari[$contatore-1] ." $mnome $mnome1 <img src='$m_squadra' /><br />";
				else echo "$contatore Non esiste file: $m_squadra MA GESTITO!  per il giocatore ".$titolari[$contatore-1] ." $mnome $mnome1<br />";
			}

			if (isset($test) and !is_file("$m_squadra")){
				if (is_file("./immagini/m_"."no_squadra".$m_estenzione)) echo "$contatore Esiste file: ./immagini/m_no_squadra".$m_estenzione." per il giocatore ".$titolari[$contatore-1] ." $mnome $mnome1 <img src='./immagini/m_no_squadra".$m_estenzione."' /><br />";
				else echo "$contatore Non esiste file: ./immagini/m_no_squadra".$m_estenzione." per il giocatore ".$titolari[$contatore-1] ." $mnome $mnome1<br />";
			}

			if (!is_file("$m_squadra") ) $m_squadra = "./immagini/m_"."no_squadra".$m_estenzione;

			if ( $gd_support=="GIF") $imm_calciatore = ImageCreateFromGif("$m_squadra");
			else $imm_calciatore = ImageCreateFromPng("$m_squadra");
			...
			if ($orientamento_campetto == 0) {
				...
				elseif ($mruolo == "1") {
					$larghezza = $lar_imm1-($lar_imm2*$cd); $larghezza = $larghezza/($cd+1); $larghezza=($larghezza+$lar_imm2)*$spazio_d-$lar_imm2;
					$altezza = ($alt_imm1-$alt_imm2)/4+$alt_p;
					$spazio_d++;
				}
				elseif ($mruolo == "2") {
					$larghezza = $lar_imm1-($lar_imm2*$cc); $larghezza = $larghezza/($cc+1); $larghezza=($larghezza+$lar_imm2)*$spazio_c-$lar_imm2;
					$altezza = ($alt_imm1-$alt_imm2)*2/4+$alt_p;
					$spazio_c++;
				}
				elseif ($mruolo == "3") {
					$larghezza = $lar_imm1-($lar_imm2*$ca); $larghezza = $larghezza/($ca+1);  $larghezza=($larghezza+$lar_imm2)*$spazio_a-$lar_imm2;
				...
				$larghezza1 = $larghezza-strlen($mnome);
			...
			elseif ($orientamento_campetto == 1) {
				...
				elseif ($mruolo == "1") {
					$altezza = $alt_imm1-($alt_imm2*$cd); $altezza = $altezza/($cd+1); $altezza = ($altezza+$alt_imm2)*$spazio_d-$alt_imm2;
					$larghezza = ($lar_imm1-$lar_imm2)/4+$alt_p;
					$spazio_d++;
				}
				elseif ($mruolo == "2") {
					$altezza = $alt_imm1-($alt_imm2*$cc); $altezza = $altezza/($cc+1); $altezza = ($altezza+$alt_imm2)*$spazio_c-$alt_imm2;
					$larghezza = ($lar_imm1-$lar_imm2)*2/4+$alt_p;
					$spazio_c++;
				}
				elseif ($mruolo == "3") {
					$altezza = $alt_imm1-($alt_imm2*$ca); $altezza = $altezza/($ca+1); $altezza = ($altezza+$alt_imm2)*$spazio_a-$alt_imm2;
		...
		elseif (($contatore > 11)  ) {
			...
			if (isset($test)) echo "Ci sono piu' di 11 giocatori! esco dalla for <br />";
			break;
		}
	}
	elseif ($riga_dati_calciatori == -1 and $contatore <= 11 ) {
		...
		if (isset($test)){
			if (is_file("$m_squadra_no")) echo "$contatore Esiste file $m_squadra_no per il giocatore ".$titolari[$contatore-1]." <img src='$m_squadra_no' /><br />";
			else echo "$contatore Non esiste file $m_squadra_no  per il giocatore  ".$titolari[$contatore-1] ."<br />";
		}

		if ( $gd_support=="GIF") $imm_calciatore = ImageCreateFromGif("$m_squadra_no");
		else $imm_calciatore = ImageCreateFromPng("$m_squadra_no");
		//
		$lar_imm2 =imagesx($imm_calciatore);
		$alt_imm2 =imagesy($imm_calciatore);
		//
...
$riga_testo=20;
ImageString($campo,1,15,$riga_testo,"Modulo: $cd-$cc-$ca",$colore_testo);

if ( (  isset($_GET['iutente']) and  isset($_GET['id_gioc']) and $iutente!="" ) or
( !isset($_GET['iutente']) and !isset($_GET['id_gioc']) and $iutente!="" )
) {$riga_testo+=10; ImageString($campo,1,15,$riga_testo,"Utente: $iutente",$colore_testo); }

if ($nome_squadra != ""  )  { $riga_testo+=10; ImageString($campo,1,15,$riga_testo,"Squadra: $nome_squadra",$colore_testo); }
if ($ultima_giornata != "") { $riga_testo+=10; ImageString($campo,1,15,$riga_testo,"Giornata: $ultima_giornata",$colore_testo); }
...
if($riduci1)
{
	if ($riduci1 < 40 or $riduci1 > 200) $riduci1 = 100;
...
if (isset($test)) {
	...
	else echo "Non esiste il file ./immagini/campo.jpg<br />";
	echo "<br />ULTERIORI VERIFICHE SQUADRE-IMMAGINI:<br />";
	if (file_exists("$percorso_cartella_dati/squadre.txt")) echo "Esiste il file $percorso_cartella_dati/squadre.txt<br /><br />";
	else echo "Non esiste il file $percorso_cartella_dati/squadre.txt<br />";
	...
	for($num1 = 0 ; $num1 < $numteam; $num1++) {
		...
		if (file_exists($file_input_imgm)) echo "Esiste il file $file_input_imgm <img src='$file_input_imgm' /><br />";
		else echo "Non esiste il file $file_input_imgm <br />";
	}
	$msquadra="no_squadra";
