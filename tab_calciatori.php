...
if ($_SESSION['valido'] == "SI" or $escludi_controllo == "SI") {
	...
	for ($num1 = 0 ; $num1 < $num_calciatori ; $num1++) {
		...
		if ($ruolo == $ruolo_guarda or $ruolo_guarda == "tutti") {
			...
			for ($num2 = 0 ; $num2 < $num_calciatori_merc ; $num2++) {
				...
				if ($numero_merc == $numero) {
					...
					if ($sec_restanti > 1) {
						...
						if ($giorni > 0) {
							if ($giorni > 1) $tempo_restante .= $giorni."gg";
							else $tempo_restante .= $giorni."gg";
						}
						if ($ore > 0) {
							if ($tempo_restante != "") $tempo_restante .= ":";
							if ($ore > 1) $tempo_restante .= $ore."o";
							else $tempo_restante .= $ore."o";
						}
						if ($minuti > 0) {
							if ($tempo_restante != "") $tempo_restante .= ":";
							if ($minuti > 1) $tempo_restante .= $minuti."m";
							else $tempo_restante .= $minuti."m";
						}
			...
			elseif ($proprietario == "<font color='navy' size='1'>Svincolato</font>" AND $stato_mercato == "P") $azione = "<a href='offerta.php?num_calciatore=$numero&amp;valutazione=$valutazione&amp;xsquadra_ok=$xsquadra_ok&amp;mercato_libero=$mercato_libero&amp;ruolos=$ruolo' class='user'>offri</a>";
