...
if ($_SESSION['valido'] == "SI") {
	...
	$linee_cambi = @file($percorso_cartella_dati."/cambi_".$_SESSION['utente']);
	...
	if ($inserire == "SI") {
		...
		for ($num = 0 ; $num < $righe_cambi ; $num++) {
			...
			if ($cambi_status == 1) {
				...
				$scrivi_file = "\n$cambi_numero,$cambi_nome,$cambi_ruolo,$cambi_valore,".$_SESSION['utente'].",$scadenza";
		...
		$fileo = file("$percorso_cartella_dati/utenti_".$_SESSION['torneo'].".php");
		list($outente, $opass, $opermessi, $oemail, $ourl, $osquadra, $otorneo, $oserie, $ocitta, $ocrediti, $ovariazioni, $ocambi, $oreg, $otitolari, $opanchina, $onome, $ocognome) = explode("<del>", trim($fileo[$_SESSION['uid']]));
		$surplus = INTVAL($ocrediti);
		$variazioni = INTVAL($ovariazioni);
		...
		$aggiorna_cambi_effettuati = $cambi_effettuati + $cambi_proposti;
		$agg_file = $outente."<del>".($opass)."<del>".$opermessi."<del>".$oemail."<del>".$ourl."<del>".$osquadra."<del>".$otorneo."<del>". $oserie."<del>".$ocitta."<del>".$aggiorna_surplus."<del>".$aggiorna_variazioni."<del>".$aggiorna_cambi_effettuati."<del>".$oreg . "<del>0<del>0<del>".$onome."<del>".$ocognome."<del>0<del>0<del>0<del>0<del>0<del>0<del>0<del>0"."\n";
		...
		if ($cambi_extra != 0) {
			...
			$linea_cce = "\n".$_SESSION['utente'].",$cambi_extra";
			flock($file_cce,LOCK_EX);
			fwrite($file_cce,$linea_cce);
			flock($file_cce,LOCK_UN);
			fclose($file_cce);
			echo "# - scrittura file controllo cambi extra<br/>";
		}
		# Cancellazione file cambi
		if (@is_file("$percorso_cartella_dati/cambi_".$_SESSION['utente'])) unlink ("$percorso_cartella_dati/cambi_".$_SESSION['utente']);
		...
		##################################
		echo "<br/><br/><br/><form method='post' action='cambi.php'>
