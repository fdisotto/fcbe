...
if ( ! file_exists( "$dir_immagini/$pic_info_file" ) ) {
    ...
    if ( $sort_images == "vecchia" )
        $sortorder = SORT_ASC; elseif ( $sort_images == "nuova" )
        $sortorder = SORT_DESC;
...
$next = $currentPic + 1;
if ( $currentPic > 0 )
    $back = $currentPic - 1; else $currentPic = "0";

$blank = empty( $item[ 1 ] ) ? '&nbsp;' : $item[ 1 ];

if ( $currentPic > 0 )
    $nav = $back; else $nav = $last;
...
function my_circular( $thumbnail_dir, $a_images, $currentPic, $thumb_row, $dir_immagini )
{
    ...
    if ( $thumb_row > $number_pics )
        $thumb_row = $number_pics;
