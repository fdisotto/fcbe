...
class DirPHP
{
    ...
    function __construct( $date_format, $protected_files = "", $header = "", $footer = "", $filetypes = "", $time_limit = 300 )
    {
        $this->load_defaults();
        $this->version = "1.1";
        $this->date_format = $date_format;
        if ( $filetypes != "" )
            $this->filetypes = array_merge( $this->filetypes, $filetypes );
        $this->protected_files = $protected_files;
        $files = explode( "/", $_SERVER[ 'PHP_SELF' ] );
        $this->protected_files[] = array_pop( $files );
    ...
    function handle_events()
    {
        ...
        if ( isset( $_GET[ 'del' ] ) ) {
            ...
            if ( isset( $_GET[ 'newname' ] ) )
                $this->rename_file( $_GET[ 'ren' ], $_GET[ 'newname' ] );
            $this->display_dir();
            if ( isset( $_GET[ 'newname' ] ) ) {
                $this->bottom_bar();
            } else {
    ...
    function display_header( $logo )
    {
        ...
        if ( $logo )
            $logo = "<img src=\"" . $logo . "\" align=\"absmiddle\" border=\"0\" alt=\"DirPHP\">";
    ...
    function display_dir()
    {
        ...
        if ( $this->use_dir != "" )
            echo "<tr height=\"24\"><td colspan=\"3\" class=\"file\" align=\"center\"><a href=\"" . $_SERVER[ 'PHP_SELF' ] . $this->parent_dir . "\"><span class=\"dir-view\">^</span></a> <span class=\"error_msg\">" . $this->use_dir . "</span></td><td class=\"file-options\">&nbsp;</td></tr>";
        //  Show error message
        $this->show_msg();
        ...
        while ( ( $file = readdir( $this->dir ) ) != false ) {
            ...
            if ( $filename == "." || $filename == ".." )
                continue;  // Ignore self and parent directory links (first two entries in each directory)
            if ( $this->settings[ 'protected_files_show' ] == 0 && $this->is_protected( $file ) )
                continue; // Don't show protected files
        ...
        if ( isset( $non_dir_files ) ) {
            ksort( $non_dir_files );
            foreach ( $non_dir_files as $value )
                echo $value;
        }
        echo "</table>\n";
        closedir( $this->dir );
    ...
    function edit_file( $file, $change_file = 0, $newfile = 0 )
    {
        if ( $change_file == 1 ) {
            ...
            if ( fwrite( $fp, $newfile ) )
                $this->msg = "File <u>" . $filename . "</u> modificato correttamente.";
            fclose( $fp );
        } else {
            if ( is_writable( "./" . $file ) ) {
