...
function chat_chk( $username, $password, &$gender, &$status )
{
    ...
    return mysql_fetch_assoc( $result ) || isset( $chat_data[ 'user' ][ $username ] ) && isset( $chat_data[ 'pass' ][ $username ] ) && ( $chat_data[ 'pass' ][ $username ] == $password );
}
...
if ( ! isset( $chat_data[ 'time' ] ) )
    $chat_data[ 'time' ] = array();
if ( ! isset( $chat_data[ 'away' ] ) )
    $chat_data[ 'away' ] = array();
if ( ! isset( $chat_data[ 'gndr' ] ) )
    $chat_data[ 'gndr' ] = array();
if ( ! isset( $chat_data[ 'stat' ] ) )
    $chat_data[ 'stat' ] = array();
if ( ! isset( $chat_data[ 'room' ] ) )
    $chat_data[ 'room' ] = array();
if ( ! isset( $chat_data[ 'user' ] ) )
    $chat_data[ 'user' ] = array();
if ( ! isset( $chat_data[ 'pass' ] ) )
    $chat_data[ 'pass' ] = array();
if ( ! isset( $chat_data[ 'data' ] ) )
    $chat_data[ 'data' ] = array();
if ( ! isset( $chat_data[ 'kick' ] ) )
    $chat_data[ 'kick' ] = array();
if ( ! isset( $chat_data[ 'mute' ] ) )
    $chat_data[ 'mute' ] = array();
...
function chat_chk_guest( $username, $password, &$gender, &$status )
{
    ...
    return ! isset( $GLOBALS[ 'chat_data' ][ 'user' ][ $username ] ) || ! isset( $GLOBALS[ 'chat_data' ][ 'pass' ][ $username ] ) || ( $GLOBALS[ 'chat_data' ][ 'pass' ][ $username ] ) == $password;
}
...
function unlog_users()
{
    ...
    foreach ( $GLOBALS[ 'chat_data' ][ 'kick' ] as $user => $tm )
        if ( $tm < time() ) {
            unset( $GLOBALS[ 'chat_data' ][ 'kick' ][ $user ] );
            $modified = true;
        }

    foreach ( $GLOBALS[ 'chat_data' ][ 'mute' ] as $user => $tm )
        if ( $tm < time() ) {
