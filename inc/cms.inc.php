...
{
    global $ptitolo, $priorita, $ptesto, $pattivo, $data_mod, $archivio_dati, $pagine_file;
    ...
    if ( $ptitolo or $ptesto ) {
        ...
        $i = 0;
        while ( $i < $num_dates && ( isset( $news_stamp ) && isset( $red_data[ "news_stamp" ] ) ) && $news_stamp < $red_data[ "news_stamp" ] ) {
...
function gestione_pagine()
{
    ...
    $now_stamp = time();
    if ( $lp )
        $start_pos_count = strip_tags( $lp ); else    $start_pos_count = 1;
    $pos_count = $start_pos_count;
    $pl_pos = 0;
    $num_news = $news_list->entries();
    if ( $start_pos_count > $num_news ) {
        $news_list->eol = true;
    }
    $news_data = array();
    ...
    do {
        ...
        if ( $page_break and $pages > 1 )
            echo "<br/><hr/><center>$nav_left | $page di $pages | $nav_right</center>";
...
function modifica_pagina()
{
    ...
    if ( $conferma == "SI" ) {
        ...
        if ( $conferma == "NO" )
            echo "<h2>Inserire titolo o testo</h2>";
        ...
        if ( $V_dati_pagine[ "pattivo" ] == "SI" )
            echo "checked";
        echo " />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		BOX (a destra)&nbsp;<input type='radio' name='pattivo' value='BOX' ";
        if ( $V_dati_pagine[ "pattivo" ] == "BOX" )
            echo "checked";
        echo " />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		LINK (a sinistra)&nbsp;<input type='radio' name='pattivo' value='LINK' ";
        if ( $V_dati_pagine[ "pattivo" ] == "LINK" )
            echo "checked";
        echo " />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		NO&nbsp;<input type='radio' name='pattivo' value='NO' ";
        if ( $V_dati_pagine[ "pattivo" ] == "NO" )
            echo "checked";
        echo " />
		<br />
...
{
    echo "<h2>Aggiungi categoria</h2>
	<form action='a_sito.php?q=7' method='post' enctype='multipart/form-data'>
	<b>titolo</b> <small>obbligatorio e univoco di massimo 40 caratteri</small><br/>
	<input type='text' name='ptitolo' size='40' maxlength='40' />
	...
	<br/>
	<input type='submit' name='submit' value='aggiungi categoria' />
	</form>";
}
...
{
    global $archivio_dati, $categorie_file, $lp;
...
function agg_notizia()
{
    ...
    if ( $start_pos_count > $num_cat ) {
        $lista_categorie->eol = true;
    }
    ...
    if ( $start_pos_count > $num_subcat ) {
        $lista_sottocategorie->eol = true;
    }
    ...
    $vedi_sottocategorie = '<select name="psottocategoria">
	...
	<b>Titolo</b> <small>obbligatorio e massimo 50 caratteri</small>
	<input type='text' name='ptitolo' size='50' maxlength='50' />
	<br /><br />
	<b>Categoria:</b> <small>elemento obbligatorio</small>
	" . $vedi_categorie . "
	<br /><br />
	<b>Appartenenza attivit&agrave;:</b> <small>elemento facoltativo</small>
	" . $vedi_sottocategorie . "
	<br /><br />
	...
	<br /><br />
	<input type='submit' name='submit' value='aggiungi notizia' />
	<br /><br />
...
function notizie()
{
    ...
    if ( $lp )
        $start_pos_count = intval( strip_tags( $lp ) ); else    $start_pos_count = 1;
    ...
    do {
        ...
        if ( $page_break and $pages > 1 )
            echo "<center>$nav_left&nbsp;&nbsp;|&nbsp;&nbsp;$page di $pages&nbsp;&nbsp;|&nbsp;&nbsp;$nav_right</center>" . $acapo;
...
function link_notizie( $categoria )
{
    ...
    if ( isset( $altri_link ) ) {
        echo "<u>Le altre notizie in questa categoria</u><br/>$altri_link";
    }
}
...
function modifica_notizia()
{
    ...
    if ( $conferma == "SI" ) {
        ...
        foreach ( $dati_categorie as $chiave => $valore ) {
            if ( $V_dati_notizie[ "pcategoria" ] == $valore[ "ptitolo" ] )
                $vedi_categorie .= "<option value='" . $valore[ "ptitolo" ] . "' selected='selected'>" . $valore[ "ptitolo" ] . "</option>"; else $vedi_categorie .= "<option value='" . $valore[ "ptitolo" ] . "'>" . $valore[ "ptitolo" ] . "</option>";
        }
