...
tabella_squadre_tra();
		...
		$num_calciatori_posseduti = 0;
			...
			$dati_calciatore = explode(",", $calciatori[$num1]);
				...
				#####################
					...
					if ($ultima_giornata > 0) $percorso = "$percorso_cartella_voti/voti$voti_ultima_giornata.txt";
				...
				$num_cer_val = count($cerca_valutazione);
					...
					$num_cervalcal = togli_acapo($num_cervalcal);
						...
						if ($ultima_giornata > 0) $stat_attivo = $dati_cervalcal[($ncs_attivo -1)];
				...
				#####################
					...
					if ($stato_mercato != "I")
					$offri = "<form method='post' action='valuta_tra.php'>
					...
					else $bgcolore = "#FFFFFF";

					if ($stat_attivo == "0") $csattivo = " - <font color=red class='piccolo'>Trasferito</font>"; else $csattivo = "";

				$tab_comprati .= "<tr bgcolor='$bgcolore'><td align='center' class='corpo'><a href='stat_calciatore.php?num_calciatore=$numero'>$numero</a></td>
				<td align='left' class='corpo'>". stripslashes($nome)." $csattivo</td>
				<td align='center' class='corpo'>$ruolo</td>
	...
	echo "<table summary='Cambi' width='100%' align='center' bgcolor='$sfondo_tab'>
	<caption>Procedura Cambi - $nome_squadra</caption>
	<tr><td width='50%' valign='top' align='center'>
	<table summary='Cambi' width='100%' cellspacing='0' cellpadding='0' style='background-color:$sfondo_tab; border=1px solid;'>
	...
	$tab_comprati</table></td><td width='50%' valign='top' align='center'>";

	#######################################
	### seconda colonna

	echo "<table summary='Cambi' cellpadding='1' width='100%'>";
	...
	$soldi_spendibili = $soldi_iniziali - $soldi_spesi + $variazioni + $surplus;
	$valuta_saldo = $valuta_saldo + $soldi_spendibili;

	echo "<tr><td>Oggi &egrave; $adesso<br/>$frase_ug<br/><br/>";
		if ($num_calciatori_posseduti != $max_calciatori) echo "<b>Attenzione!!!<br/>Rosa non completa!</b><br/>";
		...
		Valore attuale: <b>$valore_squadra</b><br/>
		FantaEuro residui: <b>$valuta_saldo</b> ($variazioni)<br/><br/>

		Cambi totali: $numero_cambi_max<br/>
		Cambi effettuati: $cambi_effettuati<br/>";

		if ($cambi_extra != 0) echo "<font class='evidenziato'>Cambi extra: $cambi_extra</font><br/>";
		if ($frase_cce) echo "<font class='evidenziato'>$frase_cce</font><br/>";
		echo "Cambi residui: $cambi_residui</td>";
		...
		$val_cal = 0;
		$cerca_giocatore = "<hr /><font class='evidenziato'>Nessun calciatore selezionato<br/>Cliccare sul nome quando evidenziato</font>";
		if ($num_calciatore) $num_ricerca = $num_calciatore;
		...
		<a href='tab_calciatori_tra.php?ruolo_guarda=P' class='user'><b>&nbsp;P&nbsp;</b></a> -
		<a href='tab_calciatori_tra.php?ruolo_guarda=D' class='user'><b>&nbsp;D&nbsp;</b></a> -
		<a href='tab_calciatori_tra.php?ruolo_guarda=C' class='user'><b>&nbsp;C&nbsp;</b></a> - ";

		if ($considera_fantasisti_come == "F") echo "<a href='tab_calciatori_tra.php?ruolo_guarda=F' class='user'><b>&nbsp;F&nbsp;</b></a> - ";

		echo "<a href='tab_calciatori_tra.php?ruolo_guarda=A' class='user'><b>&nbsp;A&nbsp;</b></a><br/><br/>";
		echo "<form method='post' action='cambi_tra.php'>
		...
		if (@is_file("$percorso_cartella_dati/cambi_tra_$nome_squadra")) $vedi_valuta = @file("$percorso_cartella_dati/cambi_tra_$nome_squadra");
		...
		$vedi = "Cessione";
		$font_color = "navy";
		$link = "<a href='valuta_tra.php?eli_val=SI&amp;num_cal=$valuta_numero' class='user'><b>X</b></a> $valuta_ruolo <a href='stat_calciatore.php?num_calciatore=$valuta_numero&amp;ruolo_guarda=$ruolo_guarda' class='user'><font color='$font_color'>$valuta_nome</font></a>";
		...
		if ($valuta_status != 0 and $valuta_status != 1) $vedi = "Errore!";
		...
		<td class='testa'>Valore<br/>acquisto</td>
		<td class='testa'>Valore<br/>attuale</td>
		...
		$cambi_tra="SI";
		if ($controllo != 0) echo "<br/><br/><a href='valuta_tra.php?eli_tutti=SI' class='user'><b>X</b></a> - Elimina tutti<br/><br/><font class='evidenziato'>Attenzione che la conferma dei cambi &egrave; definitiva ed irreversibile.<br/>La valutazione resta comunque memorizzata fino alla giornata successiva.</font>
		<center>
		<form method='post' action='inserisci_cambi_tra.php'>
		...
		<input type='hidden' name='cambi_tra' value='$cambi_tra' />
		<input type='submit' name='conferma' value='Conferma cambi' />
		</form></center>";
		} # fine if (@is_file...

		echo "<center>E' possibile valutare i cambi da effettuare. Puoi simulare i cambi selezionando i tuoi calciatori dal box di sinistra e scegliendo quelli da acquistare nel box di sopra.</center></td></tr></table>";
		echo "<br/><center>
		...
		else echo "<CENTER><h3>FASE CAMBI NON DISPONIBILE</h3>
		<br/><br/>";
