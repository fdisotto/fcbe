...
if ($_SESSION['valido'] == "SI") {
	...
	if ($chiusura_giornata != 1) {
		...
		if ($ultima_giornata >= 1 and file_exists($percorso_cartella_voti."/voti".$ultima_giornata.".txt")) $cerca = @file($percorso_cartella_voti."/voti".$ultima_giornata.".txt");
		else $cerca = @file($percorso_cartella_dati."/calciatori.txt");
		...
		if (!$nome_squadra) $nome_squadra = $_SESSION['utente'];
		...
		switch ($abilita_stat) {
			case 'AUTO': {
				if (@fopen($sito_principale.$cartella_remota.'/_stats', 'r')) $stat_1 = file_get_contents($sito_principale.$cartella_remota.'/_stats');
				else if (@fopen($sito_mirror.$cartella_remota.'/_stats', 'r')) $stat_1 = file_get_contents($sito_mirror.$cartella_remota.'/_stats');
					else $stat_1="";
			break;
			}	
			case 'PRINCIPALE': {
            ...
            case 'MIRROR': {
				if (@fopen($sito_mirror.$cartella_remota.'/_stats', 'r')) $stat_1 = file_get_contents($sito_mirror.$cartella_remota.'/_stats');
				else $stat_1="";
			break;	
			}
			case 'OFF': {
			...
			$fd = preg_replace('/\n/', ' ', $fd);
			$dati = unserialize($fd);
			if ($stat_x != $dati) {$stat_2 = fopen("./dati/_stats","w");
...
*/
					...
					if (isset($ourl) AND $ourl != "http://") $titolo .= "<br />Sito Web: <b>$ourl</b>";
					...
					for ($num2 = 0 ; $num2 < $num_calciatori ; $num2++) {
						...
						if ($proprietario == $outente) {
							...
							########################### info giocatore
								...
								else if ($abilita_stat =="NO") $stat_squadra="---------";
									else $stat_squadra = "Nessun dato";
							########################
							
							if ($attivo=="0") $attivo1="<font size='0' color='red'><b> - Trasferito</b></font>";
								else $attivo1="";
							...
							if ($sec_restanti < 1 OR ($stato_mercato != "I" and $stato_mercato != "P")) {
								...
								<td align='center'><a href='stat_calciatore.php?num_calciatore=$numero&amp;ruolo_guarda=$ruolo' class='user'>$numero</a></td>
								...
								if ($outente != $_SESSION['utente']) {
									if ($mercato_libero == "NO"){
										if ( ($stato_mercato == "I" OR $stato_mercato == "R") AND $_SESSION['utente'] == $propr_c) $azione = "Di propriet&agrave;";
										elseif ($stato_mercato == "I" AND $_SESSION['utente'] != $propr_c) $azione = "<a href='offerta.php?num_calciatore=$numero&amp;valutazione=$valutazione&amp;xsquadra_ok=$xsquadra_ok&amp;mercato_libero=$mercato_libero' class='user'>offri</a>";
										elseif ($stato_mercato == "B" AND $_SESSION['utente'] == $propr_c) $azione = "Inserito nella Busta";
										elseif ($stato_mercato == "B" AND $_SESSION['utente'] != $propr_c) $azione = "<a href='busta_offerta.php?num_calciatore=$numero&valutazione=$valutazione&xsquadra_ok=$xsquadra_ok&mercato_libero=$mercato_libero' class=user>offri</a>";
										elseif ($stato_mercato == "R" AND $proprietario == "<font class'svinc'>Svincolato</font>") $azione = "Riparazione: <a href='compra.php?num_calciatore=$numero&amp;valutazione=$valutazione&amp;xsquadra_ok=$xsquadra_ok' class='user'>compra</a>";
										elseif ($stato_mercato == "P" AND $proprietario == "<font class'svinc'>Svincolato</font>") $azione = "<a href='offerta.php?num_calciatore=$numero&amp;valutazione=$valutazione&amp;xsquadra_ok=$xsquadra_ok&amp;mercato_libero=$mercato_libero' class='user'>offri</a>";
										elseif ($stato_mercato == "R" AND $proprietario != "<font class'svinc'>Svincolato</font>") $azione = "Di terzi";
										elseif ($stato_mercato == "P" AND $_SESSION['utente'] == $propr_c) $azione = "<a href='vendi.php?num_calciatore=$numero' class='user'>vendi</a>";
										elseif ($stato_mercato == "P" AND $_SESSION['utente'] != $propr_c) $azione = "<a href='scambia.php?num_calciatore=$numero&amp;altro_utente=$outente' class='user'>scambia</a>";
										elseif ($stato_mercato == "A" AND $proprietario == "<font class'svinc'>Svincolato</font>") $azione = "<a href='compra.php?num_calciatore=$numero&amp;valutazione=$valutazione&amp;xsquadra_ok=$xsquadra_ok' class='user'>compra</a>";
										elseif ($stato_mercato == "A" AND $_SESSION['utente'] == $propr_c) $azione = "<a href='vendi.php?num_calciatore=$numero' class='user'>vendi</a>";
										elseif ($stato_mercato == "A" AND $_SESSION['utente'] != $propr_c) $azione = "<a href='scambia.php?num_calciatore=$numero&amp;altro_utente=$outente' class='user'>scambia</a>";
									}
									elseif ($mercato_libero == "SI") {
										if ($stato_mercato == "I" and $_SESSION['utente'] == $propr_c) $azione = "<a href='squadra.php' class='user'>Acquistato</a>";
										...
										elseif ($stato_mercato == "R" and $_SESSION['utente'] == $propr_c) $azione = "<a href='vedi_vendi_subito.php?num_calciatore=$numero' class='user'>vendi</a>";
										elseif ($xsquadra_ok == "NO" and $_SESSION['utente'] != $propr_c) $azione = "Riparazione: <a href='compra.php?num_calciatore=$numero&amp;valutazione=$valutazione&amp;xsquadra_ok=NO' class='user'>compra</a>";
										elseif ($stato_mercato == "A" and $_SESSION['utente'] == $propr_c) $azione = "Di propriet&agrave;";
										elseif ($stato_mercato == "A" and $_SESSION['utente'] != $propr_c) $azione = "<a href='cambi.php?num_calciatore=$numero' class='user'>cambi</a>";
									}
									else $azione = "Nessuna opzione";

									if ($stato_mercato == "C") $azione = "Mercato chiuso";
									if ($attivo == "0")  $azione = "<font class='alert'>Trasferito</font>";
									if ($num1 % 2) $colore="#FFFFFF"; else $colore="$colore_riga_alt";
									if ($blocco == 1) $azione = "<font class='alert'>Attendere aggiornamento</font>";
								...
								if ($outente == $_SESSION['utente']) {
									...
									if ($mercato_libero == "SI") {
										if ($stato_mercato == "I") $offri = "<td align='center'><a href='vedi_vendi_subito.php?num_calciatore=$numero' class='user'>vendi ora</a></td>";
										elseif ($stato_mercato == "R") $offri = "<td align='center'><a href='vedi_vendi_subito.php?num_calciatore=$numero' class='user'>svincola</a></td>";
									...
									elseif ($mercato_libero == "NO") {
										...
										if ($stato_mercato == "I") $offri = "<td align='center'>Fase iniziale</td>";
										elseif ($stato_mercato == "B" AND $tempo_off - $data_busta_chiusa == "0") $offri = "<td align='center'>Gi&agrave; acquisito nella Busta Chiusa</td>";               
										elseif ($stato_mercato == "B" AND (double)substr($tempo_off,0,12) <= (double)substr($data_busta_precedente,0,12) ) $offri = "<td align='center'>Fuori Mercato (Gia assegnato)</td>";                   
										elseif ($stato_mercato == "B" AND $tempo_off != $data_busta_chiusa) $offri = "<td align='center'><a href='busta_vendi.php?num_calciatore=$numero' class='user'>ELIMINA dalla busta</a><br><a href='busta_offerta.php?num_calciatore=$numero&amp;valutazione=$valutazione&amp;xsquadra_ok=$xsquadra_ok&amp;mercato_libero=$mercato_libero&amp;mod=SI' class='user'>MODIFICA l'offerta</a></td>";
										elseif ($stato_mercato == "R") $offri = "<td align='center'><a href='vendi.php?num_calciatore=$numero' class='user'>svincola</a></td>";
										elseif ($stato_mercato == "P") $offri = "<td align='center'><a href='vendi.php?num_calciatore=$numero' class='user'>vendi</a></td>";
										elseif ($stato_mercato == "A" OR $stato_mercato == "S") $offri = "<td align='center'><a href='vendi.php?num_calciatore=$numero' class='user'>vendi</a></td>";
					...
					if ($outente == $_SESSION['utente']) {
						...
						if ($num_calciatori_posseduti != $max_calciatori) $fuori_tabella = "<br /><b><font color='red'>ATTENZIONE: la squadra non &eacute; completa!</font></b><br />Mancano <b>$num_calciatori_comprabili</b> calciatori da acquistare.";
						else $fuori_tabella = "";
						...
						for ($num3 = 0 ; $num3 < $num_giocons ; $num3++) {
							...
							if ($composizione_squadra[$num3] == $schema_giocatori) { $xsquadra_ok = "SI"; }
						} # fine for $num3
					...
					for ($num2 = 0 ; $num2 < $num_titolari ; $num2++) {
						...
						if ($linea_comprato_P[$numero_titolare]) {
							...
							$linea_comprato_P[$numero_titolare] = ereg_replace("<tr><td>&nbsp;</td>","<tr bgcolor='$bgtabtitolari'><td class='portiere'>$num_pos</td>",$linea_comprato_P[$numero_titolare]);
							$num_pos++;
						...
						if ($linea_comprato_D[$numero_titolare]) {
							...
							$linea_comprato_D[$numero_titolare] = ereg_replace("<tr><td>&nbsp;</td>","<tr bgcolor='$bgtabtitolari'><td class='numero'>$num_pos</td>",$linea_comprato_D[$numero_titolare]);
							$num_pos++;
						...
						if ($linea_comprato_C[$numero_titolare]) {
							...
							$linea_comprato_C[$numero_titolare] = ereg_replace("<tr><td>&nbsp;</td>","<tr bgcolor='$bgtabtitolari'><td class='numero'>$num_pos</td>",$linea_comprato_C[$numero_titolare]);
							$num_pos++;
						...
						if ($linea_comprato_F[$numero_titolare]) {
							...
							$linea_comprato_F[$numero_titolare] = ereg_replace("<tr><td>&nbsp;</td>","<tr bgcolor='$bgtabtitolari'><td class='numero'>$num_pos</td>",$linea_comprato_F[$numero_titolare]);
							$num_pos++;
						...
						if ($linea_comprato_A[$numero_titolare]) {
							...
							$linea_comprato_A[$numero_titolare] = ereg_replace("<tr><td>&nbsp;</td>","<tr bgcolor='$bgtabtitolari'><td class='numero'>$num_pos</td>",$linea_comprato_A[$numero_titolare]);
							$num_pos++;
					...
					for ($num2 = 0 ; $num2 < $num_panchinari ; $num2++) {
						...
						if ($linea_comprato_P[$numero_panchinaro]) {
							...
							$linea_comprato_P[$numero_panchinaro] = ereg_replace("<tr><td>&nbsp;</td>","<tr bgcolor='$bgtabpanchinari'><td class='portiere'>$num_pos</td>",$linea_comprato_P[$numero_panchinaro]);
							$num_pos++;
						...
						if ($linea_comprato_D[$numero_panchinaro]) {
							...
							$linea_comprato_D[$numero_panchinaro] = ereg_replace("<tr><td>&nbsp;</td>","<tr bgcolor='$bgtabpanchinari'><td class='numero'>$num_pos</td>",$linea_comprato_D[$numero_panchinaro]);
							$num_pos++;
						...
						if ($linea_comprato_C[$numero_panchinaro]) {
							...
							$linea_comprato_C[$numero_panchinaro] = ereg_replace("<tr><td>&nbsp;</td>","<tr bgcolor='$bgtabpanchinari'><td class='numero'>$num_pos</td>",$linea_comprato_C[$numero_panchinaro]);
							$num_pos++;
						...
						if ($linea_comprato_F[$numero_panchinaro]) {
							...
							$linea_comprato_F[$numero_panchinaro] = ereg_replace("<tr><td>&nbsp;</td>","<tr bgcolor='$bgtabpanchinari'><td class='numero'>$num_pos</td>",$linea_comprato_F[$numero_panchinaro]);
							$num_pos++;
						...
						if ($linea_comprato_A[$numero_panchinaro]) {
							...
							$linea_comprato_A[$numero_panchinaro] = ereg_replace("<tr><td>&nbsp;</td>","<tr bgcolor='$bgtabpanchinari'><td class='numero'>$num_pos</td>",$linea_comprato_A[$numero_panchinaro]);
							$num_pos++;
					...
					if ($consenti_logo == "SI") {
						if (substr($_SESSION['url'],-3) == "jpg" or substr($_SESSION['url'],-3) == "JPG" or substr($_SESSION['url'],-3) == "gif" or substr($_SESSION['url'],-3) == "GIF") {
							...
							if (@is_file($logo_squadra)) $dati_utente .= "<img src='$logo_squadra' border='1' alt='Logo' align='right' width='100' />";
						}
					}
					...
					if ($_SESSION['url'] and $_SESSION['url'] != "http://" and substr($_SESSION['url'],-3)!="jpg" and substr($_SESSION['url'],-3)!="JPG" and substr($_SESSION['url'],-3)!="gif" and substr($_SESSION['url'],-3)!="GIF") $dati_utente .= "&nbsp;-&nbsp;<a href=".$_SESSION['url']." target='_blank' class='user'>Sito web</a>";

					$dati_utente .= "<br />Presidente: ".$_SESSION['utente'];

					if ($_SESSION['citta']) $dati_utente .= "<br />Citt&agrave;: ".$_SESSION['citta'];

					$dati_utente .= "<br />Email: ".$_SESSION['email']."<br />Data iscrizione: ".$_SESSION['reg'];

					if ($mercato_libero == "SI" ) $dati_utente .= "<br /><br />Cambi effettuati: <b>$cambi_effettuati</b><br />Cambi totali: <b>$numero_cambi_max</b>.";
					...
					if ($mercato_libero != "SI" ) $dati_utente .= "<br /><a href='#off' class='user'>Offerte in corso</a>";
					...
					if ($outente == $_SESSION['utente']) {
						...
						if ($tab_lato != "" and $xsquadra_ok != "SI") echo "$fuori_tabella\n";
						elseif ($vedi_campetto == "SI") echo "<table summary='fantacampo' align='center'><tr>
						...
						if (substr($ourl,-3) == "jpg" or substr($ourl,-3) == "JPG" or substr($ourl,-3) == "gif" or substr($ourl,-3)== "GIF") {
							...
							$logo_squadra = "./immagini/loghi/$outente.jpg";
							if (@is_file($logo_squadra)) echo "</td><td><img alt='Logo squadra' src='$logo_squadra' border='1' />";
						}
						echo "</td></tr></table>";
					}
					
					if ($outente == $_SESSION['utente']) echo "<table summary='squadra corpo' bgcolor='$sfondo_tab' width='100%' align='center' cellpadding='3'>
					<tr><td valign='top'>
					...
					elseif ($outente != $_SESSION['utente']) echo "<table summary='squadra corpo' bgcolor='$sfondo_tab' width='100%' align='center' cellpadding='3'>
					<tr><td valign='top'>
					$tab_centro
					$fuori_tabella
					</td></tr></table>";
					...
					if ($mercato_libero != "SI" AND $stato_mercato != "R" AND $stato_mercato != "B") echo "<a name='off'></a><h3><u>Vedi le offerte in corso</u></h3><br />$tab_offerte";
