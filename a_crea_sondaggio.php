...
if ( $_SESSION[ 'valido' ] == "SI" and $_SESSION[ 'permessi' ] >= 4 ) {
    ...
    if ( ! @is_file( $percorso_cartella_dati . "/sondaggio.php" ) ) {
        if ( isset( $crea_sondaggio ) ) {
            ...
            if ( ( preg_replace( "/[0-9]/", "", $num_voti_segreti ) != "" or $num_voti_segreti == "" or $num_voti_segreti == 0 ) and $tipo == "votazione_segreta" )
                $crea = "NO";
            if ( ( preg_replace( "/[0-9]/", "", $num_voti_palesi ) != "" or $num_voti_palesi == "" or $num_voti_palesi == 0 ) and $tipo == "votazione_palese" )
                $crea = "NO";
            if ( isset( $crea ) && $crea != "NO" ) {
                if ( $tipo == "votazione_segreta" or $tipo == "sondaggio_segreto" )
                    $voto_palese = "NO"; else $voto_palese = "SI";
                if ( $tipo == "votazione_segreta" )
                    $voti_consentiti = $num_voti_segreti;
                if ( $tipo == "votazione_palese" )
                    $voti_consentiti = $num_voti_palesi;
                if ( $tipo == "sondaggio_segreto" or $tipo == "sondaggio_palese" )
                    $voti_consentiti = 0;
...
<center><input type='submit' name='crea_sondaggio' value='Crea il sondaggio' /></center>
        ...
        if ( isset( $cancella_sondaggio ) ) {
            if ( unlink( $percorso_cartella_dati . "/sondaggio.php" ) )
                echo "Sondaggio cancellato"; else echo "ERRORE.<br/>";
            echo "<form method='post' action='./a_crea_sondaggio.php'>
<input type='submit' name='vai' value='OK' />
</form>";
        } # fine if ($cancella_sondaggio)
        else {
            if ( isset( $nuova_opzione ) ) {
                require_once $percorso_cartella_dati . "/sondaggio.php";
                ...
                for ( $num1 = 0; $num1 < $num_linee_sondaggio; $num1++ ) {
                    if ( substr( $linee_sondaggio[ $num1 ], 0, 10 ) == "\$opzioni =" ) {
                        if ( count( $opzioni ) != 0 )
                            $linee_sondaggio[ $num1 ] = str_replace( ");", ",);", $linee_sondaggio[ $num1 ] );
                    ...
                    if ( $linea == "?>" )
                        $linee_sondaggio[ $num1 ] = "\$voti" . ( count( $opzioni ) + 1 ) . " = 0;\n?>";
                } # fine for $num1
            ...
            if ( $voto_palese == "SI" )
                echo "palese"; else echo "segret$o";
            if ( $voti_consentiti > 1 )
                echo " ($voti_consentiti voti consentiti)";
            echo "</center><br/>";
