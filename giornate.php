...
if ($_SESSION['valido'] == "SI") {
	...
	if ($_GET['opzione'] != 2 AND $_GET['opzione'] != 3) $opzione = 3;
	else $opzione = $_GET['opzione'];

	if ($opzione == 2){
		if ($chiusura_giornata >= 1) {
		if (!$_GET['nome_squadra']) $nome_squadra = "tutti";
		else $nome_squadra = $_GET['nome_squadra'];
			...
			if ($nome_squadra == "tutti" OR $nome_squadra == $outente) {
				...
				if ($nome_squadra){
					...
					for ($num3 = $num_iniziale; $num3 < $num_messaggi; $num3++) {
						...
						if ($stato_mercato != "I" AND strpos($nome,$nome_squadra) !== false) $messmerc .= "$nome<br/>";
					} # fine for $num3

					$messmerc = ereg_replace("Radio mercato: ","",$messmerc);
					$messmerc = ereg_replace("$nome_squadra ha","",$messmerc);
				}
				########################
				# Layout pagina
	...
	#######################
	elseif ($opzione == 3){
		echo "<div style='background-color: $sfondo_tab; margin-top:5px; margin-bottom:5px; padding: 5px; border: 1px solid $sfondo_tab2'>Giornate giocate:&nbsp;".$acapo;

		for($num1 = 1 ; $num1 < 40 ; $num1++) {
			if (strlen($num1) == 1) $num1 = "0".$num1;
			$giornata_controlla = "giornata$num1";
			if (!@is_file($percorso_cartella_dati."/".$giornata_controlla."_".$_SESSION['torneo']."_0")) break;
			else {
		...
		if ($giornata) $file_giornata = file($percorso_cartella_dati."/giornata".$giornata."_".$_SESSION['torneo']."_0");
		...
		for($num1 = $num_colonne ; $num1 < 2; $num1++) $tab_formazioni .= "<td>&nbsp;</td>".$acapo;
		$tab_formazioni .= "</tr>";
		$tipo_campionato = "";
		...
		if ($tipo_campionato == "S") {
			...
			for ($num1 = 0 ; $num1 < $num_scontri ; $num1++) {
				...
				if ($_SESSION['utente'] == $dati_scontri[0]) echo "<b>";
				echo $nome_squadra_memo[$dati_scontri[0]];
				if ($_SESSION['utente'] == $dati_scontri[0]) echo "</b>";
				echo " - ";
				if ($_SESSION['utente'] == $dati_scontri[1]) echo "<b>";
