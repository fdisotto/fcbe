...
if ( $_SESSION[ 'valido' ] == "SI" and $_SESSION[ 'permessi' ] >= 4 ) {
    ...
    if ( isset( $copia_file_voti ) and isset( $posizione_file ) ) {
    ...
    if ( isset( $ripristina_originale ) && $ripristina_originale == "SI" ) {
        ...
        if ( ! copy( $filego, $nuovofilego ) ) {
            echo "Copia di $filego non riuscita ...\n";
        }
        $ripristina_originale = "NO";
    }

    if ( isset( $cancella_giornata ) && $cancella_giornata == "SI" ) {
        $filego = $percorso_cartella_dati . "/giornata" . $giornata . "_" . $n_torneo . "_" . $n_serie;
        if ( file_exists( $percorso_cartella_voti . "/voti" . $giornata . ".txt" ) ) {
            unlink( $percorso_cartella_voti . "/voti" . $giornata . ".txt" );
            if ( ! unlink( $filego ) ) {
                echo "Cancellazione di $filego non riuscita ...\n";
            }
        }
        $ripristina_originale = "NO";
    }
    ...
    for ( $num = 1; $num < $num_tornei; $num++ ) {
        ...
        unset( $outente, $opass, $opermessi, $oemail, $ourl, $osquadra, $otorneo, $oserie, $ocitta, $ocrediti, $ovariazioni, $ocambi, $oreg );
        ...
        if ( file_exists( $percorso_cartella_dati . "/giornata" . $tgiornata . "_" . $otid . "_" . $otserie ) ) {
        ...
        for ( $num1 = 0; $num1 < $num_linee_file_giornata; $num1++ ) {
            ...
            if ( $linea == "#@& fine voti #@&" )
                $leggendo_voti = "NO";
            if ( ! empty( $leggendo_voti ) && $leggendo_voti == "SI" ) {
                $voti[ $num2 ] = $linea;
                $num2++;
        ...
        for ( $num1 = 1; $num1 < $linee; $num1++ ) {
            @list( $outente, $opass, $opermessi, $oemail, $ourl, $osquadra, $otorneo, $oserie, $ocitta, $ocrediti, $ovariazioni, $ocambi, $oreg ) = explode( "<del>", $fileu[ $num1 ] );
        ...
        if ( ! empty( $voti_esistenti ) && $voti_esistenti == "SI" ) {
            echo "<br/><table summary='Calcoli' width='100%' bgcolor=$sfondo_tab><caption>Voti della $num_giornata giornata</caption><tr><td valign='top'>
			<u><b>Voti</b></u><br/>";
            $num_voti = count( $voti );
            ...
            if ( $tipo_campionato != "N" ) {
                ...
                for ( $num1 = 1; $num1 < $linee; $num1++ ) {
                    @list( $ooutente, $oopass, $oopermessi, $ooemail, $oourl, $oosquadra, $ootorneo, $ooserie, $oocitt�, $oocrediti, $oovariazioni, $oocambi, $ooreg ) = explode( "<del>", $fileu[ $num1 ] );
                    if ( $oopermessi >= 0 )
                        echo $ooutente . ": " . $punti[ $ooutente ] . "<br/>";
                } # fine for $num1

                echo "</td><td align='center' valign='top'><b>Giornata chiusa.</b>";

                if ( @is_file( $percorso_cartella_dati . "/giornata" . $tgiornata . "_" . $otorneo . "_" . $oserie . "_iniziale" ) )
                    echo "<br/><form method='post' action='a_giornata.php'>
				...
				<input type='submit' name='calcola' value='Ripristina originale $tgiornata' />
				</form>
				<font class='evidenziato'>ATTENZIONE<br/>
				Proseguendo si ripristina il file giornata<br/> senza il calcolo dei voti!<br/>NON CI SONO CONTROLLI ULTERIORI!!!</font>";

                if ( @is_file( $percorso_cartella_dati . "/giornata" . $tgiornata . "_" . $otorneo . "_" . $oserie ) )
                    echo "<br/><br/><br/><form method='post' action='a_giornata.php'>
				<input type='hidden' name='num_giornata' value='$num_giornata' />
				<input type='hidden' name='giornata' value='$tgiornata' />
				<input type='hidden' name='n_torneo' value='$otorneo' />
				<input type='hidden' name='n_serie' value='$oserie' />
				<input type='hidden' name='cancella_giornata' value='SI' />
				<input type='submit' name='cancella' value='Cancella giornata $tgiornata' />
				</form>
				<font class='evidenziato'>ATTENZIONE<br/>
				Proseguendo si cancella il file giornata<br/>NON CI SONO CONTROLLI ULTERIORI!!!</font>";
			...
			</form>"; elseif ( intval( $giornata ) == intval( $num_giornata ) and @is_file( $percorso_cartella_voti . "/voti" . $giornata . ".txt" ) ) {
                echo "<br/><center><form method='post' action='a_calcola_punti_giornata.php'>
				<input type='hidden' name='num_giornata' value='$num_giornata' />
				<input type='hidden' name='giornata' value='$giornata' />
				<input type='submit' name='calcola' value='Calcola i punteggi $num_giornata $giornata' />
                ...
                if ( intval( $giornata ) == intval( $num_giornata ) and ( $prima_parte_pos_file_voti or $num_giornata_file_voti == "SI" ) ) {
                    ...
                    if ( $num_giornata_file_voti == "SI" ) {
                        ...
                        if ( $num_giornata_file_voti_doppio == "SI" ) {
                            if ( strlen( $num_giornata_a ) == 1 )
                                $num_giornata_a = "0" . $num_giornata_a;
                    ...
                    echo "<center><form method='POST' action='a_giornata.php'>
					<input type='hidden' name='num_giornata' value='$num_giornata' />
					<input type='hidden' name='giornata' value='$giornata' />
					<input type='submit' name='copia_file_voti' value='Copia i voti $giornata $num_giornata' />
					dal file <input type='text' name='posizione_file' value='$posizione_file' size='65' />
					</form></center><br /><br /><br />";
