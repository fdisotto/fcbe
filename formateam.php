...
if ( $_SESSION[ 'valido' ] == "SI" ) {
    ...
    if ( $ultima_giornata == "" )
        echo "<br/><br/><center><h3>Statistiche non presenti</h3></center><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/>"; else {
        $vedi_squadra = $_SESSION[ 'utente' ];
        echo "<center><h3><u>Riassunto squadra di $vedi_squadra</u></h3>Dati statistici dalla giornata di campionato 01 alla giornata $ultima_giornata</center><br/>";

        $tabella = "<table width='100%' cellpadding='0' cellspacing='0' align='center' bgcolor='$sfondo_tab' summary='Tabella statistiche squadra'>
		<tr><td class='testa'>Num.</td>
		<td class='testa'>Nome</td>
		<td class='testa'>Squadra</td>
		...
		<td class='testa'>Ultimi voti</td></tr>";

        #######################################

        if ( $stato_mercato != "I" and $ultima_giornata >= 1 )
            $voti = file( "$percorso_cartella_voti/voti$ultima_giornata.txt" ); else $voti = file( "$percorso_cartella_dati/calciatori.txt" );
        ...
        for ( $knum1 = 0; $knum1 <= $num_cer_squ; $knum1++ ) {
            ...
            if ( $xsquadra == $vedi_squadra ) {
                        ...
                        for ( $num2 = 0; $num2 <= $num_voti; $num2++ ) {
                            ...
                            if ( $num_calciatore == $num_calciatore_voto ) {
                                ...
                                if ( $statistiche == "SI" ) {
                                    ...
                                    $tot_golsegnati = ( $tot_golsegnati ?? 0 ) + $stat_golsegnati;
                                    $tot_golsubiti = ( $tot_golsubiti ?? 0 ) + $stat_golsubiti;
                                }
                                break;
                ...
                */
                if ( $statistiche == "SI" ) {
                    if ( $stat_attivo == 0 )
                        $mess = "<b><font color=red>Non disponibile</font></b>"; else $mess = "In attivit�";
                    ...
                    if ( $stat_attivo == "0" )
                        $csattivo = " - <font class='piccolo'>Trasferito</font>"; else $csattivo = "";

                    $lmsquadra = "m_" . strtolower( $stat_squadra ) . ".png";
                    $squadra = "<a href='tab_squadre.php?vedi_squadra=$stat_squadra' style=' border: 0px; text-decoration: none;' title='$stat_squadra'><img src='./immagini/$lmsquadra' style='width: 25px; border: 0px; text-decoration: none;' alt='$stat_squadra' /></a>";

                    $tabella .= "<tr bgcolor='$color'>
					<td align='center'><a href='stat_calciatore.php?num_calciatore=$numero&amp;ruolo_guarda=$ruolo_guarda' class='user'>$numero</a></td>
					<td>$nome $csattivo</td>
					...
					<td align='center'>$stat_valore</td>
					<td align='center'>$uvt ($uvg)</td></tr>";
