...
if ($chiusura_giornata == 1) echo "<a href='./registro_mercato.php' >Registro mercato</a>".$acapo;
	...
	if ($considera_fantasisti_come == "F") echo "<option value='F'> i fantasisti</option>$acapo";
	...
	if ($stato_mercato != "I" AND $ultgio >=1) {
		...
		if ($considera_fantasisti_come == "F") echo "<input type='radio' name='ruolo_guarda' value='F' /> F |$acapo";
		echo "<input type='radio' name='ruolo_guarda' value='A' /> A$acapo
		</form>$acapo";

		if ($statistiche == "SI") {

			if ($ultgio > 2 ) echo "<hr /><form method='post' action='suggteam.php'>$acapo
			...
			for($num1 = 0 ; $num1 < $numteam; $num1++) {
				echo "<option value='".trim($team[$num1])."'>".ucfirst(strtolower(trim($team[$num1])))."</option>$acapo";
			} # fine for $num1

			echo "</select><input type='submit' name='guarda_squadra' value='Vedi' /></form>$acapo";
